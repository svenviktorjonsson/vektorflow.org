...
const isTree=process.argv.includes('--tree');
...
<script src="/previews/0.6.0/compiled/runtime-stones-7/vf-compiled-runtime-bridge.js"></script>
<script type="module">
import {readMechanicalAsset,prepareMechanicalInitialState} from '/previews/0.6.0/compiled/runtime-stones-7/vf-world-mechanical-runtime.mjs';
import {createMechanicalWorldGpu} from '/previews/0.6.0/compiled/runtime-stones-7/vf-world-mechanical-gpu.mjs';
import {createWorldSceneEmbeddingGpu} from '/previews/0.6.0/compiled/runtime-stones-7/vf-world-scene-embedding-gpu.mjs';
const check=(x,m)=>{if(!x)throw Error(m)};let device;
try{
 const base='/previews/0.6.0/compiled/stones-granite-7/';
 const runtime=VfCompiledRuntimeBridge.instantiateWasmRuntime({bytes:new Uint8Array(await(await fetch(base+'main.wasm')).arrayBuffer()),manifest:await(await fetch(base+'manifest.json')).json()});runtime.init();
 ...
 let encoder,z=null,expected=null,wind=null;
 ...
 encoder=device.createCommandEncoder();embedding.render(encoder,isTree?{pos:[8,-17,7],target:[0,0,3.7],fov:42}:{pos:[3.4,-5.6,3.2],target:[0,0,.95],fov:42},{grass:isTree,particles:false});
 ...
 await fetch('/result',{method:'POST',body:JSON.stringify({passed:true,adapter:adapter.info,z,expected,wind,png})});embedding.destroy();physics.destroy();device.destroy();
...
const server=createServer(async(req,res)=>{try{
 ...
 if(route==='/test'){res.setHeader('Content-Type','text/html');res.end(isTree?html.replaceAll('runtime-stones-7','runtime-tree-8').replaceAll('stones-granite-7','tree-leaves-8'):html);return;}
...
const args=['--headless=new','--enable-gpu','--no-first-run','--no-default-browser-check','--window-size=900,800','--user-data-dir='+path.join(work,'profile'),'http://127.0.0.1:'+server.address().port+'/test'];
