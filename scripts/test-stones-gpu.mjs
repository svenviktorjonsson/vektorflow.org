...
<script src="/previews/0.6.0/compiled/runtime-stones-11/vf-compiled-runtime-bridge.js"></script>
<script type="module">
import {readMechanicalAsset,prepareMechanicalInitialState} from '/previews/0.6.0/compiled/runtime-stones-11/vf-world-mechanical-runtime.mjs';
import {createMechanicalWorldGpu} from '/previews/0.6.0/compiled/runtime-stones-11/vf-world-mechanical-gpu.mjs';
import {createWorldSceneEmbeddingGpu} from '/previews/0.6.0/compiled/runtime-stones-11/vf-world-scene-embedding-gpu.mjs';
const check=(x,m)=>{if(!x)throw Error(m)};let device;
try{
 const base='/previews/0.6.0/compiled/stones-mixed-11/';
 ...
 if(!isTree){const droppedBody=4;check(initial.bodies[droppedBody*20+16]<initial.bodies[1*20+16],'Regression requires the smallest stone');physics.setHeld(droppedBody,4);encoder=device.createCommandEncoder();physics.placeHeld(encoder);device.queue.submit([encoder.finish()]);await device.queue.onSubmittedWorkDone();physics.setHeld(-1,0);
 ...
 const state=await physics.readBodies();z=state[droppedBody*20+2];expected=4+world.gravity[2]*world.time_step**2*dropSteps*(dropSteps+1)/2;
 ...
 physics.setHeld(droppedBody,4);encoder=device.createCommandEncoder();physics.placeHeld(encoder);device.queue.submit([encoder.finish()]);await device.queue.onSubmittedWorkDone();physics.setHeld(-1,0);
 ...
 check(minimumVerticalSpeed<-1,'Stone never reached impact speed: '+JSON.stringify(stoneDynamics));check(maximumUpwardSpeed>.05&&maximumAngularSpeed>.03,'Irregular stone contact produced no bounce/rotation response: '+JSON.stringify(stoneDynamics));check(minimumFloorClearance>-.002,'Stone tunneled through ground: '+JSON.stringify(stoneDynamics));check(sleepingBodies===initial.bodyCount&&linearSpeed<.01&&angularSpeed<.01,'Dropped stone did not sleep after settling: '+JSON.stringify(stoneDynamics));
 check(maximumBaseSpeed<.25&&maximumBaseDisplacement<.08,'Small stone moved supported multi-ton bases unrealistically: '+JSON.stringify(stoneDynamics));
 ...
 if(!isTree){
  ...
  physics.reset();physics.setHeld(4,4);encoder=device.createCommandEncoder();physics.placeHeld(encoder);device.queue.submit([encoder.finish()]);await device.queue.onSubmittedWorkDone();physics.setHeld(-1,0);
