...
const geometry=planTreeGeometryReference(createTreeGeometryPlannerReference(identity,{splitDepth:7,lateralShoots:true,trunkShoots:false,foliageDensity:.42,scaffoldBranches:2}),forest,{treeIndices:[0],detailLevels:[2],primitiveBudget:2400});
...
const tree=adaptTreeRenderPacketToWebGpuMeshesReference(retained.packets[0],{vertexBudget:393216,indexBudget:2359296,leafContact:true});
...
const target=new URL('../public/previews/0.6.0/live/tree/assets/',import.meta.url);await mkdir(target,{recursive:true});await writeFile(new URL('tree-mesh.bin.gz',target),gzipSync(output,{level:9}));
