...
for(const {id,files} of applications){
  ...
  if(id==='wheel')for(const world of program.gpu_worlds){verifyInitialExclusion(world,arenas.find(a=>a.layer.id===world.layer_id));const kernel=runtime.readBinding(`${world.binding_prefix??`$world$gpu$${world.world_id}`}$physics`);for(const name of ['motion_find_toi','motion_bar_toi','preventive_predict','motion_update_ledger','motion_guard_end','motion_rotate'])assert.ok(kernel.includes(`fn ${name}`),`Compiled ${world.kind} missing ${name}`);assert.match(kernel,/bitcast<u32>\(fraction\)&0x7fffffffu/);}
  if(id==='tree'){
...
const page=await readFile(new URL('coming-soon.html',root),'utf8');assert.equal((page.match(/<iframe /g)||[]).length,3);assert.doesNotMatch(page,/id="hourglass"|Water and sand wheel|simulation-tabs/);
console.log('Three compiled applications, water-only wheel and complete module closure verified');
