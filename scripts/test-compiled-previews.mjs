...
for(const {id,files} of applications){
  ...
  if(id==='tree'){
    ...
    const world=program.gpu_worlds[0];for(const url of [world.solid_properties.asset,...Object.values(world.solid_properties.variants)]){
      const asset=decodeMeshes(await readFile(new URL(url.slice(1),root))),{initial,meshes}=prepare(structuredClone(world),arenas,asset);assert.equal(initial.nodes.length,initial.nodeCount*16);assert.equal(initial.geometry.length,initial.nodeCount*20);assert.ok(initial.geometry.every(Number.isFinite));assert.ok(meshes.every(m=>m.vertices.every(Number.isFinite)&&m.vertices.length%24===0));
      const leafIndex=asset.findIndex(m=>m.id.includes('foliage')),leafCount=asset[leafIndex].leaf_vertex_count??12,leaves=meshes[leafIndex].vertices;for(let base=0;base<leaves.length;base+=leafCount*24){const anchor=[0,1,2].map(a=>(leaves[base+a]+leaves[base+24+a])*.5);for(let j=0;j<leafCount;j++){assert.equal(leaves[base+j*24+19],1);for(let a=0;a<3;a++)assert.ok(Math.abs(leaves[base+j*24+16+a]-anchor[a])<1e-6);assert.ok(leaves[base+j*24+23]>0);}}
    }const kernel=runtime.readBinding(`$world$gpu$${program.gpu_worlds[0].world_id}$physics`);for(const law of ['air_velocity','obstacle_normal','coupled_neighbor'])assert.ok(kernel.includes(`fn ${law}`),`Compiled tree missing ${law}`);assert.equal(program.gpu_worlds[0].properties.turbulence_intensity,.16);console.log('Four tree variants: coherent air, material-aware modes, finite geometry and shared leaf anchors verified');
  }
  ...
  if(id==='wheel'){assert.deepEqual(program.gpu_worlds.map(world=>world.kind),['liquid','granular']);assert.ok(program.views.every(view=>view.controls.start_paused===true),'Both wheel material Views must start paused');assert.equal(program.gpu_worlds[0].geometry.radius,.5);assert.ok(arenas[2].layer.count>=1808);assert.ok(arenas[5].layer.count>=1870);}
