...
let bundlePromise;
const previewBundle=()=>bundlePromise??=fetch('./previews/0.6.0/compiled/bundle.json?v=wheel-granular-12').then(r=>{if(!r.ok)throw Error('Build receipt unavailable');return r.json();});
...
function mountSourceTabs({ id, files }) {
  ...
  const sourceUrl=name=>`./sources/coming-soon/${id}/${name}?v=${id==='stones'?'granite-8':id==='tree'?'wind-9':'wheel-granular-12'}`;
