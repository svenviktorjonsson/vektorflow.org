const DEFAULT_COLORS = Object.freeze({
  ...
  water: Object.freeze([0.28, 0.52, 0.50, 1]),
  foam: Object.freeze([0.93, 0.97, 0.95, 1]),
  accent: Object.freeze([0.61, 0.91, 0.87, 1]),
});
...
fn composite_fragment(input: FullscreenOut) -> @location(0) vec4<f32> {
  ...
  let reflected_air = params.air_color.rgb * 1.75
    + params.accent_color.rgb * (0.08 + 0.12 * sin(world.x * 4.3));
...
export async function createLiquidParticleEmbeddingGpu(deviceArgument, canvasArgument,
  ...
  const primaryPipeline = await device.createRenderPipelineAsync({
  ...
  const diffusePipeline = await device.createRenderPipelineAsync({
