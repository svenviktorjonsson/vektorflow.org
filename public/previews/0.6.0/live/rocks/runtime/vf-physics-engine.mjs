...
/**
 * Advance rigid contact worlds in either two or three spatial dimensions.
 * The 2D path retains the established polygon solver. The 3D path uses the
 * same generalized impulse model with [r]x represented by the skew-symmetric
 * cross-product matrix instead of the planar 90-degree rotation operator.
 */
export function stepRigidBodyWorld(world, dt) {
  const dimension = Number(world.dimension ?? world.dimensions ?? (
    world.planes?.some((plane) => plane.normal?.length === 3)
      || world.bodies?.some((body) => body.position?.length === 3) ? 3 : 2
  ));
  if (dimension === 2) return stepRigidPolygonWorld2D(world, dt);
  if (dimension === 3) return stepRigidConvexWorld3D(world, dt);
  throw new RangeError('rigid body world dimension must be 2 or 3');
}

export function applyRigidBodyImpulse3D(body, {
  linearImpulse = [0, 0, 0], angularImpulse = [0, 0, 0],
} = {}) {
  const linear = vector(linearImpulse, 3);
  const angular = vector(angularImpulse, 3);
  const normalized = normalizeRigidBody3D(body, 0);
  const inverseInertia = worldInverseInertia3(normalized);
  applyRigidImpulse3(normalized, inverseInertia, linear, [0, 0, 0], angular);
  const wake = body.sleeping === true
    && (linear.some((value) => value !== 0) || angular.some((value) => value !== 0));
  return {
    ...body,
    velocity: [...normalized.velocity],
    angularVelocity: [...normalized.angularVelocity],
    ...(wake ? { sleeping: false, sleep_time: 0 } : {}),
  };
}

export function rigidBodyPointInverseMass3D(body, { point, normal } = {}) {
  const contactPoint = vector(point, 3);
  const contactNormal = vector(normal, 3);
  const normalLength = Math.hypot(...contactNormal);
  if (!(normalLength > 1e-15)) throw new RangeError('rigid contact normal must be nonzero');
  const unitNormal = rigidScale3(contactNormal, 1 / normalLength);
  const normalized = normalizeRigidBody3D(body, 0);
  const inverseInertia = worldInverseInertia3(normalized);
  const matrix = rigidCollisionMatrix3(normalized.inverseMass, inverseInertia,
    rigidSub3(contactPoint, normalized.position));
  return rigidDot3(unitNormal, rigidMatVec3(matrix.pp, unitNormal));
}

function stepRigidConvexWorld3D(world, dt) {
  const step = finiteNonNegative(dt, 'dt');
  const gravity = vector(world.gravity ?? [0, 0, -9.82], 3);
  const maximumStep = finitePositive(world.maxStep ?? world.step_dt ?? 1 / 120, 'maxStep');
  const substeps = Math.max(1, Math.ceil(step / maximumStep));
  const h = substeps ? step / substeps : 0;
  const iterations = Math.max(1, Math.trunc(Number(world.solverIterations
    ?? world.solver_iterations ?? 8)));
  const sleepLinearThreshold = finiteNonNegative(world.sleepLinearThreshold
    ?? world.sleep_linear_threshold ?? 0.1, 'sleep linear threshold');
  const sleepAngularThreshold = finiteNonNegative(world.sleepAngularThreshold
    ?? world.sleep_angular_threshold ?? 0.3, 'sleep angular threshold');
  const sleepDelay = finiteNonNegative(world.sleepDelay ?? world.sleep_delay ?? 0.5,
    'sleep delay');
  const sleepContactGrace = finiteNonNegative(world.sleepContactGrace
    ?? world.sleep_contact_grace ?? 0.05, 'sleep contact grace');
  const bodies = (world.bodies || []).map(normalizeRigidBody3D);
  const planes = (world.planes || []).map(normalizeRigidPlane3D);
  const contacts = [];
  for (let substep = 0; substep < substeps; substep += 1) {
    for (const body of bodies) {
      body.hadContact = false;
      if (body.inverseMass === 0 || body.sleeping) continue;
      body.velocity = rigidAdd3(body.velocity, rigidScale3(gravity, h));
      body.position = rigidAdd3(body.position, rigidScale3(body.velocity, h));
      body.orientation = integrateWorldQuaternion3(body.orientation, body.angularVelocity, h);
    }
    for (let iteration = 0; iteration < iterations; iteration += 1) {
      for (const body of bodies) {
        if (body.inverseMass === 0) continue;
        for (const plane of planes) {
          const contact = resolveRigidPlaneContact3D(body, plane);
          if (contact && iteration === 0) contacts.push(Object.freeze({ ...contact, substep }));
        }
      }
    }
    for (const body of bodies) {
      ...
      const belowThreshold = Math.hypot(...body.velocity) <= sleepLinearThreshold
        && Math.hypot(...body.angularVelocity) <= sleepAngularThreshold;
  ...
  return {
    ...
    dimension: 3,
    gravity,
    planes: planes.map(({ authored }) => ({ ...authored })),
    bodies: bodies.map((body) => ({
      ...
      sleep_time: body.sleepTimer,
      sleep_contact_time: body.contactMemory,
    })),
    contacts,
  };
}

function normalizeRigidBody3D(body, index) {
  const vertices = body.localVertices ?? body.local_vertices;
  if (!Array.isArray(vertices) || vertices.length < 4) {
    throw new TypeError(`rigid body ${body.id ?? index} requires at least four 3D localVertices`);
  }
  const localVertices = vertices.map((point) => vector(point, 3));
  const fixed = body.fixed === true || body.static === true || body.mass === Number.POSITIVE_INFINITY;
  const mass = fixed ? Number.POSITIVE_INFINITY : finitePositive(body.mass ?? 1, 'body mass');
  const inertia = normalizeRigidInertia3(body.inertia ?? body.inertia_tensor ?? [1, 1, 1], fixed);
  const muS = finiteNonNegative(body.mu_s ?? body.static_friction ?? body.friction ?? 0.65,
    'body mu_s');
  const muD = finiteNonNegative(body.mu_d ?? body.dynamic_friction ?? body.friction
    ?? Math.min(muS, 0.45), 'body mu_d');
  ...
  return {
    ...
    localVertices,
    position: vector(body.position ?? [0, 0, 0], 3),
    velocity: vector(body.velocity ?? [0, 0, 0], 3),
    orientation: normalizedQuaternion3(body.orientation ?? [1, 0, 0, 0]),
    angularVelocity: vector(body.angularVelocity ?? body.angular_velocity ?? [0, 0, 0], 3),
    inverseMass: fixed ? 0 : 1 / mass,
    inverseInertiaBody: inertia,
    material: {
      eN: unitInterval(body.e_n ?? body.normal_restitution ?? body.restitution ?? 0.35,
        'body e_n'),
      ...
      contactRadius: finitePositive(body.contact_radius ?? Math.max(...localVertices.map(
        (point) => Math.hypot(...point))), 'body contact_radius'),
      restitutionThreshold: finiteNonNegative(body.restitution_threshold ?? 0.5,
...
function normalizeRigidInertia3(inertia, fixed) {
  if (fixed) return [[0, 0, 0], [0, 0, 0], [0, 0, 0]];
  if (Array.isArray(inertia) && inertia.length === 3 && inertia.every(Number.isFinite)) {
    return inertia.map((value, row) => {
      const inverse = 1 / finitePositive(value, 'body inertia');
      return [row === 0 ? inverse : 0, row === 1 ? inverse : 0, row === 2 ? inverse : 0];
    });
  }
  if (Array.isArray(inertia) && inertia.length === 3
      && inertia.every((row) => Array.isArray(row) && row.length === 3)) {
    return invertMatrix3(inertia);
  }
  throw new TypeError('3D rigid inertia must be a positive diagonal vector or 3x3 tensor');
}

function normalizeRigidPlane3D(plane, index) {
  const sourceNormal = vector(plane.normal, 3);
  const length = Math.hypot(...sourceNormal);
  if (!(length > 1e-12)) throw new RangeError(`rigid plane ${plane.id ?? index} requires a normal`);
  const normal = rigidScale3(sourceNormal, 1 / length);
  const offset = plane.offset == null
    ? rigidDot3(normal, vector(plane.point ?? [0, 0, 0], 3))
    : finiteNumber(plane.offset, 'plane offset') / length;
  const muS = finiteNonNegative(plane.mu_s ?? plane.static_friction ?? plane.friction ?? 0.65,
    'plane mu_s');
  const muD = finiteNonNegative(plane.mu_d ?? plane.dynamic_friction ?? plane.friction
    ?? Math.min(muS, 0.45), 'plane mu_d');
  if (muD > muS) throw new RangeError(`rigid plane ${plane.id ?? index} requires mu_d <= mu_s`);
  return {
    authored: { ...plane, normal: [...normal], offset },
    normal,
    offset,
    material: {
      eN: unitInterval(plane.e_n ?? plane.normal_restitution ?? plane.restitution ?? 0.35,
        'plane e_n'),
      eT: unitInterval(plane.e_t ?? plane.tangential_restitution ?? 0, 'plane e_t'),
      muS,
      muD,
      muR: finiteNonNegative(plane.mu_r ?? plane.rolling_friction ?? 0, 'plane mu_r'),
      contactRadius: finitePositive(plane.contact_radius ?? 1, 'plane contact_radius'),
      restitutionThreshold: finiteNonNegative(plane.restitution_threshold ?? 0.5,
        'plane restitution_threshold'),
    },
  };
}

function resolveRigidPlaneContact3D(body, plane) {
  const centerDistance = rigidDot3(body.position, plane.normal) - plane.offset;
  if (centerDistance >= body.material.contactRadius) return null;
  const rotation = quaternionMatrix3(body.orientation);
  let support = null;
  let signedDistance = Number.POSITIVE_INFINITY;
  for (const local of body.localVertices) {
    const point = rigidAdd3(body.position, rigidMatVec3(rotation, local));
    const distance = rigidDot3(point, plane.normal) - plane.offset;
    if (distance < signedDistance) {
  ...
  body.position = rigidAdd3(body.position, rigidScale3(plane.normal, penetration));
  const contact = rigidAdd3(support, rigidScale3(plane.normal, penetration));
  const radius = rigidSub3(contact, body.position);
  const contactVelocity = rigidAdd3(body.velocity, rigidCross3(body.angularVelocity, radius));
  const normalSpeed = rigidDot3(contactVelocity, plane.normal);
  const material = mixRigidMaterial3(body.material, plane.material);
  let impulse = [0, 0, 0];
  let angularImpulse = [0, 0, 0];
  let impact = false;
  if (normalSpeed < 0) {
    const inverseInertia = worldInverseInertia3(body, rotation);
    const matrix = rigidCollisionMatrix3(body.inverseMass, inverseInertia, radius);
    impact = -normalSpeed > material.restitutionThreshold;
    const restitution = impact ? material.eN : 0;
    const effectiveNormalMass = rigidDot3(plane.normal,
      rigidMatVec3(matrix.pp, plane.normal));
    const normalMagnitude = effectiveNormalMass > 1e-12
      ? -(1 + restitution) * normalSpeed / effectiveNormalMass : 0;
    const normalImpulse = rigidScale3(plane.normal, Math.max(0, normalMagnitude));
    impulse = normalImpulse;
    applyRigidImpulse3(body, inverseInertia, normalImpulse, radius, [0, 0, 0]);
    const afterNormalVelocity = rigidAdd3(body.velocity,
      rigidCross3(body.angularVelocity, radius));
    const tangentVelocity = rigidSub3(afterNormalVelocity,
      rigidScale3(plane.normal, rigidDot3(afterNormalVelocity, plane.normal)));
    const tangentSpeed = Math.hypot(...tangentVelocity);
    if (tangentSpeed > 1e-9 && normalMagnitude > 0) {
      const tangent = rigidScale3(tangentVelocity, 1 / tangentSpeed);
      const effectiveTangentMass = rigidDot3(tangent, rigidMatVec3(matrix.pp, tangent));
      const desired = effectiveTangentMass > 1e-12
        ? -(1 + material.eT) * tangentSpeed / effectiveTangentMass : 0;
      const staticLimit = material.muS * normalMagnitude;
      const frictionMagnitude = Math.abs(desired) <= staticLimit
        ? desired
        : Math.max(-material.muD * normalMagnitude,
          Math.min(material.muD * normalMagnitude, desired));
      const frictionImpulse = rigidScale3(tangent, frictionMagnitude);
      impulse = rigidAdd3(impulse, frictionImpulse);
      applyRigidImpulse3(body, inverseInertia, frictionImpulse, radius, [0, 0, 0]);
    }
    const rollingVelocity = rigidSub3(body.angularVelocity,
      rigidScale3(plane.normal, rigidDot3(body.angularVelocity, plane.normal)));
    const rollingSpeed = Math.hypot(...rollingVelocity);
    if (rollingSpeed > 1e-9 && normalMagnitude > 0 && material.muR > 0) {
      const rollingAxis = rigidScale3(rollingVelocity, 1 / rollingSpeed);
      const angularMass = rigidDot3(rollingAxis, rigidMatVec3(inverseInertia, rollingAxis));
      const desired = angularMass > 1e-12 ? -rollingSpeed / angularMass : 0;
      const limit = material.muR * material.contactRadius * normalMagnitude;
      const magnitude = Math.max(-limit, Math.min(limit, desired));
      angularImpulse = rigidScale3(rollingAxis, magnitude);
      applyRigidImpulse3(body, inverseInertia, [0, 0, 0], radius, angularImpulse);
    }
  }
  return Object.freeze({
    bodyId: body.authored.id ?? null,
    planeId: plane.authored.id ?? null,
    point: Object.freeze([...contact]),
    normal: Object.freeze([...plane.normal]),
    penetration,
    normalSpeed,
    impulse: Object.freeze([...impulse]),
    angularImpulse: Object.freeze([...angularImpulse]),
    impact,
  });
}

function rigidCollisionMatrix3(inverseMass, inverseInertia, radius) {
  const wedge = rigidSkew3(radius);
  const rotational = rigidMatMul3(rigidMatMul3(wedge, inverseInertia), wedge);
  return {
    pp: rigidMatSub3(rigidScaleMat3(rigidIdentity3(), inverseMass), rotational),
    pL: rigidScaleMat3(rigidMatMul3(wedge, inverseInertia), -1),
    Lp: rigidMatMul3(inverseInertia, wedge),
    LL: inverseInertia,
  };
}

function mixRigidMaterial3(a, b) {
  return {
    eN: Math.max(a.eN, b.eN),
    eT: Math.max(a.eT, b.eT),
    muS: Math.sqrt(a.muS * b.muS),
    muD: Math.sqrt(a.muD * b.muD),
    muR: Math.sqrt(a.muR * b.muR),
    contactRadius: Math.min(a.contactRadius, b.contactRadius),
    restitutionThreshold: Math.max(a.restitutionThreshold, b.restitutionThreshold),
  };
}

function applyRigidImpulse3(body, inverseInertia, impulse, radius, angularImpulse) {
  body.velocity = rigidAdd3(body.velocity, rigidScale3(impulse, body.inverseMass));
  body.angularVelocity = rigidAdd3(body.angularVelocity, rigidMatVec3(inverseInertia,
    rigidAdd3(rigidCross3(radius, impulse), angularImpulse)));
}

function worldInverseInertia3(body, rotation = quaternionMatrix3(body.orientation)) {
  return rigidMatMul3(rigidMatMul3(rotation, body.inverseInertiaBody), rigidTranspose3(rotation));
}

function normalizedQuaternion3(value) {
  const q = vector(value, 4);
  const length = Math.hypot(...q);
  if (!(length > 1e-12)) throw new RangeError('rigid orientation quaternion must be non-zero');
  return q.map((component) => component / length);
}

function integrateWorldQuaternion3(value, angularVelocity, dt) {
  const [w, x, y, z] = value;
  const [wx, wy, wz] = angularVelocity;
  return normalizedQuaternion3([
...
function quaternionMatrix3([w, x, y, z]) {
  return [
...
function invertMatrix3(matrix) {
  ...
  if (!Number.isFinite(determinant) || Math.abs(determinant) < 1e-15) {
    throw new RangeError('3D rigid inertia tensor must be invertible');
  }
  return [
...
const rigidAdd3 = (a, b) => a.map((value, axis) => value + b[axis]);
const rigidSub3 = (a, b) => a.map((value, axis) => value - b[axis]);
const rigidScale3 = (value, amount) => value.map((component) => component * amount);
const rigidDot3 = (a, b) => a[0] * b[0] + a[1] * b[1] + a[2] * b[2];
const rigidCross3 = (a, b) => [
  a[1] * b[2] - a[2] * b[1],
  a[2] * b[0] - a[0] * b[2],
  a[0] * b[1] - a[1] * b[0],
];
const rigidSkew3 = ([x, y, z]) => [[0, -z, y], [z, 0, -x], [-y, x, 0]];
const rigidIdentity3 = () => [[1, 0, 0], [0, 1, 0], [0, 0, 1]];
const rigidMatVec3 = (matrix, value) => matrix.map((row) => rigidDot3(row, value));
const rigidTranspose3 = (matrix) => matrix[0].map((_, column) => matrix.map((row) => row[column]));
const rigidMatMul3 = (a, b) => a.map((row) => rigidTranspose3(b).map((column) =>
  rigidDot3(row, column)));
const rigidScaleMat3 = (matrix, amount) => matrix.map((row) =>
  row.map((value) => value * amount));
const rigidMatSub3 = (a, b) => a.map((row, y) => row.map((value, x) => value - b[y][x]));
