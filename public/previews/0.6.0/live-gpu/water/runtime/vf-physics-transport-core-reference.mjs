...
export function stepPhysicsStateBufferReference(state, steps = 1, {
  ...
  resolveBoundary = null, afterStep = null, applyGravity = true } = {}) {
  if (!Number.isInteger(steps) || steps < 0 || steps > maximumSteps) throw new RangeError(diagnostic);
  for (let step = 0; step < steps; step += 1) {
    for (let element = 0; element < state.count; element += 1) {
      const offset = element * 3;
      if (applyGravity) for (let axis = 0; axis < 3; axis += 1) {
        state.velocities[offset + axis] += state.gravity[axis] * state.timeStep;
      }
      for (let axis = 0; axis < 3; axis += 1) {
        state.positions[offset + axis] += state.velocities[offset + axis] * state.timeStep;
      }
