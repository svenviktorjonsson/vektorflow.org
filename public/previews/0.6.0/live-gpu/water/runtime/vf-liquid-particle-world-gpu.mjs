...
fn advect(@builtin(global_invocation_id) invocation: vec3<u32>) {
  ...
  particles[index].position = contact.position;
  particles[index].velocity = contact.velocity;
  record_motion_telemetry(contact.position, contact.velocity);
}
...
fn update_diffuse_and_render(@builtin(global_invocation_id) invocation: vec3<u32>) {
  ...
  marker.position = contact.position;
  marker.velocity = contact.velocity;
...
export async function createLiquidParticleWorldGpuRuntime(deviceArgument, options = {}) {
  ...
  const seed = createLiquidParticleWorldGpuSeedReference(options);
  ...
  const solidGpuPacket = createLiquidParticleWorldGpuSolidPacketReference(seed, {
  ...
  const paramsBuffer = createBuffer(device, 'VKF Liquid world parameters',
    128, GPUBufferUsage.UNIFORM | copyDst);
  const shader = device.createShaderModule({
    label: 'VKF 2D Liquid particle world GPU specialization',
    code: LIQUID_PARTICLE_WORLD_GPU_WGSL,
  });
  ...
  const entryPoints = ['predict', 'clear_cells', 'fill_cells', 'sort_cells', 'divergence_lambda',
    ...
    'apply_filter', 'update_diffuse_and_render'];
  ...
  const updateParams = (timeStep) => {
    ...
    paramsF32.set([policy.bedSlope, policy.bedAtStone,
      policy.stoneCenterX, policy.stoneHalfWidth], 24);
  ...
  const buildGrid = (pass) => {
    dispatch(pass, 'clear_cells', cellItemCount);
  ...
  return Object.freeze({ kind: 'liquid-particle-world-gpu-runtime:v1', device,
    ...
    gridCellCount, pipelines: Object.freeze(pipelines), step, stepMany,
    ...
    readTelemetry, resetTelemetry, reset, destroy,
