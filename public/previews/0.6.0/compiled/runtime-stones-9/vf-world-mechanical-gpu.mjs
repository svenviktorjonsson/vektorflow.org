    ...
    for(var i=0u;i<params.counts.x;i++){
      ...
      if(depth>0.0&&im>0.0&&bodies[i].info.w<0.5){
        ...
        if(cv.z<0.0){
          ...
          angular-=up*sign(spin_velocity)*spin_delta;bodies[i].w=vec4<f32>(angular,0.0);
        }
      }
      for(var j=i+1u;j<params.counts.x;j++){
        ...
        if(vn<0.0){
          ...
          let tangent_mass=total+select(dot(li_tangent,li_tangent)/bodies[i].info.y,0.0,im==0.0)+select(dot(lj_tangent,lj_tangent)/bodies[j].info.y,0.0,jm==0.0);
          ...
          let angular_inverse=select(1.0/bodies[i].info.y,0.0,im==0.0)+select(1.0/bodies[j].info.y,0.0,jm==0.0);
          ...
          if(im>0.0){bodies[i].w=vec4<f32>(bodies[i].w.xyz+angular_impulse/bodies[i].info.y,0.0);}if(jm>0.0){bodies[j].w=vec4<f32>(bodies[j].w.xyz-angular_impulse/bodies[j].info.y,0.0);}
        }
      }
    }
  }
