...
export async function createMaterialWorld(device,canvas,compiled,world,arena,engineeringOptions={}) {
  ...
  contactResources.rotatingTrajectory=engineeringOptions.rotatingTrajectory??(world.kind==='liquid'&&physicsSource.includes('fn motion_enter_rotating_frame'));
  ...
  const physics=await factory(device,{initialState,solidPacket,geometry:world.geometry,
    shaderSource:physicsSource,preventiveContact:contactResources,forceGeometryCaching:engineeringOptions.forceGeometryCaching??contactResources.rotatingTrajectory});
  ...
  return {world,physics,embedding,boundary,contact,time:0,accumulator:0,paused:startPaused,revision:0,angle:world.geometry.rotation,targetAngle:world.geometry.rotation,logicalAngle:world.geometry.rotation,
    reset(){this.revision++;physics.reset();contact.reset();this.time=0;this.accumulator=0;this.paused=startPaused;this.angle=world.geometry.rotation;this.targetAngle=this.angle;this.logicalAngle=this.angle;
      physics.setWheel({angle:world.geometry.rotation,angularVelocity:0});},
