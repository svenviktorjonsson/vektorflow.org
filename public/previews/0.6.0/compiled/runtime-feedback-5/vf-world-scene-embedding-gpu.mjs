...
struct Vertex { @location(0) p:vec3<f32>, @location(1) n:vec3<f32>, @location(2) color:vec4<f32>, @location(3) tag:f32, @location(4) compliance:f32, @location(5) emission:vec3<f32>, @location(6) anchor:vec3<f32>, @location(7) leaf:f32, @location(8) hinge:vec3<f32>, @location(9) length:f32 };
...
fn posed(v:Vertex)->Out {
  ...
  if(scene.flags.x>0.5&&v.compliance>0.0){
    if(v.leaf>0.5){let offset=p-v.anchor;let angle=clamp(dot(mode_displacement(v.anchor,1u),v.n)/max(v.length,0.02),-1.1,1.1);
      p=v.anchor+hinge_rotate(offset,v.hinge,angle)+displacement(v.anchor)*v.compliance;n=hinge_rotate(n,v.hinge,angle);
...
@fragment fn material_fragment(v:Out)->@location(0) vec4<f32>{
  let n=normalize(v.n);let l=normalize(scene.sun.xyz-v.p);let projected=scene.light_vp*vec4<f32>(v.p+n*0.002,1.0);
  let ndc=projected.xyz/projected.w;let uv=ndc.xy*vec2<f32>(0.5,-0.5)+0.5;
  var shadow=1.0;
  if(all(uv>=vec2<f32>(0.0))&&all(uv<=vec2<f32>(1.0))&&ndc.z>=0.0&&ndc.z<=1.0){shadow=textureSampleCompareLevel(shadow_texture,shadow_sampler,uv,ndc.z-0.00002);}
...
export async function createWorldSceneEmbeddingGpu(device,canvas,world,physics,meshes,shaderSource){
  ...
  const attributes=[{shaderLocation:0,offset:0,format:'float32x3'},{shaderLocation:1,offset:12,format:'float32x3'},{shaderLocation:2,offset:24,format:'float32x4'},{shaderLocation:3,offset:40,format:'float32'},{shaderLocation:4,offset:44,format:'float32'},{shaderLocation:5,offset:48,format:'float32x3'},{shaderLocation:6,offset:64,format:'float32x3'},{shaderLocation:7,offset:76,format:'float32'},{shaderLocation:8,offset:80,format:'float32x3'},{shaderLocation:9,offset:92,format:'float32'}];
