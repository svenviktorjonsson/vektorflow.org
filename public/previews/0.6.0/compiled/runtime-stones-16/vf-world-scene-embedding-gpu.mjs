...
@fragment fn material_fragment(v:Out)->@location(0) vec4<f32>{
  ...
  if(all(uv>=vec2<f32>(0.0))&&all(uv<=vec2<f32>(1.0))&&ndc.z>=0.0&&ndc.z<=1.0){
    if(scene.flags.x<0.5){
      // One contiguous PCF penumbra. The old sparse edge-distance rings
      // quantized the silhouette into two visible borders. The blocker gap
      // sets filter radius; dense taps keep one continuous transition.
      let dimensions=vec2<f32>(textureDimensions(shadow_texture));
      let pixel=clamp(vec2<i32>(uv*dimensions),vec2<i32>(0),vec2<i32>(dimensions)-1);
      var blockerDepth=1.0;
      for(var direction=0;direction<5;direction++){
        let axis=array<vec2<i32>,5>(vec2<i32>(0),vec2<i32>(-2,0),vec2<i32>(2,0),vec2<i32>(0,-2),vec2<i32>(0,2))[direction];
        let depth=textureLoad(shadow_texture,clamp(pixel+axis,vec2<i32>(0),vec2<i32>(dimensions)-1),0);
        if(depth<ndc.z-0.00008){blockerDepth=min(blockerDepth,depth);}
      }
      ...
      let pixelWidth=2.0*receiver*tan(25.0*3.141592654/180.0)/dimensions.x;
      let radius=clamp(0.65+gap*scene.domain_span.w/max(blocker,0.1)/pixelWidth,0.65,1.5);
      shadow=0.0;
      for(var y=0;y<4;y++){for(var x=0;x<4;x++){
        let offset=(vec2<f32>(f32(x),f32(y))-vec2<f32>(1.5))*radius/1.5;
        shadow+=textureSampleCompareLevel(shadow_texture,shadow_sampler,uv+offset/dimensions,ndc.z-0.00008);
      }}
      shadow*=0.0625;
    }else{
