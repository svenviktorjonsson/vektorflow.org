...
export function prepareMechanicalInitialState(world,arenas,asset){
  ...
  // visual offset made correctly settled stones appear to float above it.
  meshes.push({shadow:false,vertices:new Float32Array([[-radius,-radius],[radius,-radius],[radius,radius],[-radius,radius]].flatMap(([x,y])=>[x,y,-0.0001,0,0,1,...color,-1,0,...Array(12).fill(0)])),indices:new Uint32Array([0,1,2,0,2,3])});
  p.lights=[];
...
export async function bootMaterialWorlds(compiled){
  ...
  const release=event=>{event.preventDefault();pointers.delete(event.pointerId);pinchDistance=0;if(drag?.id===event.pointerId){
    if(drag.kind==='stone'){
      // Release only removes the kinematic hold. Rotation is generated by
      // r × impulse in the shared rigid contact law on impact.
      physics.releaseHeld();
    }drag=null;
