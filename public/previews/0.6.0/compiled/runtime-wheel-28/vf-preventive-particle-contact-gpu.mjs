...
@compute @workgroup_size(128)
fn motion_advance_particles(@builtin(global_invocation_id) gid:vec3<u32>){
  let i=gid.x;
  if(i>=motion.counts.x||atomicLoad(&motion_control[6])!=0u||motion_value(9)<=0.0){return;}
  var state=motion_state(i);
  let duration=select(motion_value(9),0.0,motion_ledger()&&atomicLoad(&motion_control[18])!=0u);
  let start=motion_position_parts(i,0.0);
  var parts=motion_position_parts(i,duration);
  if(atomicLoad(&motion_control[57])!=0u){
    let delta=motion_value(42);
    parts=motion_rotated_parts(parts,delta);
    let difference=motion_compensated(parts.xy,parts.zw,-start.xy);
    let displacement=motion_two_sum(difference.xy,difference.zw-start.zw);
    state=vec4<f32>(parts.xy,(displacement.xy+displacement.zw)/motion_value(9));
  }else{state=vec4<f32>(parts.xy,state.zw);}
  // Liquid wall shear is a bounded momentum exchange with the moving rim.
  // It acts once per accepted interval, after CCD has certified the position.
  // Unlike setting v_t = omega*r, the exponential viscosity step cannot
  // overshoot either the current tangential speed or the wall speed.
  if(motion.counts.y==3u&&duration>0.0&&motion.material.y>0.0){
    let offset=parts.xy-motion.wheel.xy;
    let distance=length(offset);
    let gap=WHEEL_RADIUS-motion.minimum.z-WHEEL_BAR_HALF_WIDTH-distance;
    let support=motion.material.z;
    if(distance>1.0e-8&&gap<support){
      let tangent=vec2<f32>(-offset.y,offset.x)/distance;
      let weight=clamp(1.0-max(0.0,gap)/support,0.0,1.0);
      let fraction=1.0-exp(-motion.material.y*weight*duration);
      let relative_tangent=dot(state.zw,tangent)-motion_value(3)*distance;
      state=vec4<f32>(state.xy,state.zw-tangent*relative_tangent*fraction);
    }
  }
  motion_particles[i*motion.counts.y]=bitcast<vec4<u32>>(state);
  let previous=motion_actors[i*128u+122u];
  motion_actors[i*128u+122u]=vec4<u32>(bitcast<vec2<u32>>(parts.zw),previous.zw);
}
...
fn motion_solve_cell(bucket:u32,color:u32){
  ...
  for(var slot=0u;slot<count;slot++){
    ...
    if(radius>=WHEEL_RADIUS-clearance-motion.material.w*8.0){motion_wall_impulse(i,-offset/max(radius,1.0e-20),vec2<f32>(0.0),select(motion.material.z,0.0,motion.counts.y==3u));}
...
export async function createPreventiveParticleContactGpu(device,world,physics,source,resources){
  ...
  const params=new ArrayBuffer(96),u=new Uint32Array(params),f=new Float32Array(params);u.set([r.count,world.kind==='granular'?2:3,0,world.geometry.segments.length,r.side,r.side,64,0]);f.set([...r.minimum,r.radius,r.cellWidth],8);f.set([...world.geometry.center,0,0],12);
  const sand=world.kind==='granular',projectedSand=sand&&!r.sandBarrier;
  const liquidSpacing=world.properties.spacing??0.009;
  const liquidShearRate=(world.properties.viscosity??0)/(world.properties.density??1000)/(liquidSpacing*liquidSpacing);
  f.set([world.properties.restitution??0,sand?(world.properties.friction??.9):liquidShearRate,
    sand?(world.properties.wall_friction??.78):liquidSpacing*2.35,r.margin],16);
