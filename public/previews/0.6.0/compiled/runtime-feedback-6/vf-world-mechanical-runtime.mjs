...
export function prepareMechanicalInitialState(world,arenas,asset){
  ...
  if(kind==='rigid'){
    ...
    const nodeArena=arenas.find(a=>a.layer.id===world.nodes_layer_id);initial.nodeCount=nodeArena.layer.count;initial.nodes=new Float32Array(initial.nodeCount*16);initial.geometry=new Float32Array(initial.nodeCount*12);
    ...
    for(let i=0;i<initial.nodeCount;i++){
      ...
      // Rooted lawn and woody canopy have different local material modes.
      const coefficients=z===1?{mass:world.grass_properties.mass??0.08,stiffness:world.grass_properties.spring_constant??40,damping:world.grass_properties.damping??0.8}:beam??{mass:world.nodes_properties.mass??1,stiffness:world.nodes_properties.spring_constant??60,damping:world.nodes_properties.damping??12};
      initial.geometry.set([coefficients.mass,coefficients.stiffness,coefficients.damping,0],(initial.nodeCount+i)*4);
    }
    ...
    for(const item of asset){const vertices=convert(item.vertices,-1,(v,d)=>Math.pow(Math.max(0,v[d+2])/8,2)*0.65,(v,d)=>{
      ...
      if(item.id?.includes('foliage')){
        // Each accepted procedural leaf has twelve vertices. Its first pair
        // straddles the petiole attachment and its last vertex is the apex.
        if(vertices.length%(12*24))throw new Error('Invalid procedural leaf topology');
        for(let base=0;base<vertices.length;base+=12*24){
          const anchor=[0,1,2].map(a=>(vertices[base+a]+vertices[base+24+a])*.5);
          const tip=Array.from(vertices.subarray(base+11*24,base+11*24+3));
          const axis=tip.map((v,a)=>v-anchor[a]),length=Math.hypot(...axis),normal=Array.from(vertices.subarray(base+3,base+6));
          const hinge=[normal[1]*axis[2]-normal[2]*axis[1],normal[2]*axis[0]-normal[0]*axis[2],normal[0]*axis[1]-normal[1]*axis[0]],norm=Math.hypot(...hinge);
          const c=cell(...anchor),o=(initial.nodeCount*2+c)*4;
          initial.geometry[o]+=(world.solid_properties.leaf_mass??0.0003);initial.geometry[o+1]+=(world.solid_properties.leaf_spring_constant??0.3);initial.geometry[o+2]+=(world.solid_properties.leaf_damping??0.006);
          for(let j=0;j<12;j++){const d=base+j*24;vertices.set(anchor,d+16);vertices[d+19]=1;vertices.set(hinge.map(v=>v/Math.max(norm,1e-8)),d+20);vertices[d+23]=length;vertices[d+11]=Math.pow(Math.max(0,anchor[2])/8,2)*0.65;}
        }
      }
      meshes.push({vertices,indices:item.indices});}
    p.grass_count=world.grass_properties.count??81920;p.parcel_mass=p.mass??(p.density??1.225)*span.reduce((v,n)=>v*n,1)/initial.parcelCount;p.node_mass=beam?.mass??world.nodes_properties.mass??1;p.spring_constant=beam?.stiffness??(world.nodes_properties.spring_constant??60)/(world.nodes_properties.elasticity??1);p.damping=beam?.damping??world.nodes_properties.damping??12;p.beam=beam;
