...
export async function createLiquidParticleWorldGpuRuntime(deviceArgument, options = {}) {
  ...
  const shader = device.createShaderModule({
    ...
    code: options.shaderSource ?? (LIQUID_PARTICLE_WORLD_GPU_WGSL + SWEPT_WHEEL_CONTACT_WGSL + PREVENTIVE_PARTICLE_CONTACT_WGSL),
  });
  ...
  const predictForces=(encoder)=>{
    ...
    const run=name=>{pass.setPipeline(preventivePipelines[name]);pass.dispatchWorkgroups(ceilDiv(seed.count,128));};
    for(let i=0;i<policy.divergenceIterations;i++){run('divergence_lambda');run('apply_pressure');}
    run('predict');run('classify_and_filter');run('apply_filter');
    for(let i=0;i<policy.densityIterations;i++){run('density_lambda');run('apply_pressure');}
    pass.end();
  };
