...
fn elastic_step(@builtin(global_invocation_id) gid:vec3<u32>){
  ...
  if(arrayLength(&nodes)>params.counts.z){
    ...
    if(leaf.x>0.0&&leaf.y>0.0){
      let leaf_force=force*0.65-leaf.y*nodes[j].d.xyz;
      let leaf_v=(leaf.x*nodes[j].v.xyz+leaf_force*dt)/(leaf.x+dt*leaf.z+dt*dt*leaf.y);
      nodes[j].v=vec4<f32>(leaf_v,0.0);nodes[j].d=vec4<f32>(nodes[j].d.xyz+leaf_v*dt,0.0);
    }
  }
}
`;
