...
export function prepareMechanicalInitialState(world,arenas,asset){
  ...
  if(kind==='rigid'){
    ...
    initial.bodyCount=asset.length;initial.hullCount=hullCount;initial.bodies=new Float32Array(asset.length*20);initial.geometry=new Float32Array(asset.length*hullCount*4);
...
export async function bootMaterialWorlds(compiled){
  ...
  canvas.addEventListener('pointermove',event=>{event.preventDefault();
    ...
    if(drag.kind==='stone'){const metres=2*drag.depth*Math.tan(camera.fov*Math.PI/360)/canvas.getBoundingClientRect().height;physics.setHeld(drag.body,Math.max(drag.z,Math.min(4,drag.z+(drag.y-event.clientY)*metres)));}
