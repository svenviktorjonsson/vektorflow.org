...
fn wind_step(@builtin(global_invocation_id) gid:vec3<u32>){
  ...
  if(inside&&object.w>0.0&&(object.w<1.5||p.p.z<0.22)&&p.v.w==0.0){
    ...
    // Intercept only the blade area represented by this coarse cell, rather
    // than collide every parcel with a metre-wide opaque canopy voxel.
    let coverage=select(1.0,clamp(attached.z/cellArea,0.0,1.0),attached.x>0.0);
    let outgoing=vec3<f32>(incoming.x*0.55,incoming.y+(unit(i*17u)-0.5)*params.wind.x*0.18,incoming.z+params.wind.x*0.035);
    p.v=vec4<f32>(mix(incoming,outgoing,coverage),0.10);
    p.p.w=0.25;
    let impulse=(incoming-p.v.xyz)*params.wind.y;
    let impacted=select(owner,c+params.grid.x*params.grid.y,object.w>1.5);
...
fn elastic_step(@builtin(global_invocation_id) gid:vec3<u32>){
  ...
  var v=(material.x*old_v+(force+coupling*k*0.3-total_k*nodes[i].d.xyz)*dt)/(material.x+dt*material.z+dt*dt*total_k);
  ...
  if(arrayLength(&nodes)>params.counts.z){
    ...
    if(leaf.x>0.0&&leaf.y>0.0){
      ...
      let rhs=(material.x+mass)*old_v+moment*angular_v+(force+coupling*k*0.3-total_k*nodes[i].d.xyz)*dt;
