...
export const GRANULAR_VISUAL_SAMPLES_PER_GUIDE = 40;
...
const VISUAL_SAMPLES_PER_GUIDE: u32 = 40u;
const VISUAL_EMITTED: u32 = 0x80000000u;
const VISUAL_DORMANT: u32 = 0x40000000u;
...
fn grain_vertex(@builtin(vertex_index) vertex_index: u32,
  ...
  let sample_radius = physical_radius / sqrt(f32(VISUAL_SAMPLES_PER_GUIDE));
  let visible_radius = select(max(sample_radius, pixel_world * 0.50),
    max(physical_radius, pixel_world * 0.72), particle_mode);
  let emitted = (grain.contact_count & VISUAL_EMITTED) != 0u;
  let render_velocity = select(vec2<f32>(0.0), grain.velocity,
    particle_mode || emitted);
  let speed = length(render_velocity);
  let motion_direction = select(vec2<f32>(0.0, 1.0), render_velocity / max(speed, 1.0e-6),
    speed > 1.0e-6);
  ...
  output.position = vec4<f32>(world_to_clip(world_position),
    0.0, 1.0);
  output.local = local;
  // Each optical sample represents exactly 1/N of its guide's projected
  // material area, including when a subpixel grain needs a larger raster disk.
  output.area_scale = select(min(1.0,
    sample_radius * sample_radius / (visible_radius * visible_radius)),
    1.0, particle_mode);
  output.id = grain.id;
  output.contact_count = grain.contact_count;
  output.world_position = world_position;
  output.velocity = render_velocity;
  return output;
}
...
fn density_vertex(@builtin(vertex_index) vertex_index: u32,
  ...
  output.airborne = select(0.0, 1.0, grain.velocity.y < -0.04);
  return output;
}
...
fn material_composite_fragment(input: FullscreenOut) -> @location(0) vec4<f32> {
  ...
  // The density above excludes airborne guide mass. Only supported material
  // reconstructs a continuous surface; finite visual shares draw freefall.
...
fn grain_fragment(input: GrainOut) -> @location(0) vec4<f32> {
  if (params.canvas.z < 0.5
      && (input.contact_count & VISUAL_DORMANT) != 0u) { discard; }
  ...
  if (!particle_mode) {
    let uv = input.position.xy / params.canvas.xy;
    let local_field = field_at(uv);
    let packed = smoothstep(1.05, 1.65,
      max(local_field.x - local_field.w, 0.0));
    // In a packed bed, broad optical disks expose the guide lattice. Retain
    // only sparse per-sample facets; restore full grains in actual flow.
    let flowing = smoothstep(0.12, 0.40, length(input.velocity));
    let facet = pixel_noise(floor((input.local + vec2<f32>(1.0)) * 12.0),
      input.id);
    let packed_facet = 0.01 + smoothstep(0.90, 0.99, facet) * 0.45;
    coverage *= mix(1.0, packed_facet + flowing * 0.12, packed);
  }
  if (coverage <= 0.001) { discard; }
  if (!particle_mode && sand_solid_at(input.world_position)) { discard; }
  ...
  if (particle_mode) {
    ...
    color = mineral_color(input.id) * (0.78 + 0.26 * stable_unit(input.id))
      * (0.75 + 0.45 * micro);
    color += vec3<f32>(0.86,0.84,0.76) * smoothstep(0.985,0.999,micro)
      * (0.12 + 0.12 * moving) * params.light.w;
  }
...
const VISUAL_SAMPLES_PER_GUIDE: u32 = 40u;
const VISUAL_LANES_PER_GUIDE: u32 = 4u;
const VISUAL_EMITTED: u32 = 0x80000000u;
const VISUAL_DORMANT: u32 = 0x40000000u;
fn visual_hash_u32(value: u32) -> u32 {
  var x = value;
  x ^= x >> 16u;
  x *= 0x7feb352du;
  x ^= x >> 15u;
...
fn visual_hash_unit(value: u32) -> f32 {
  return f32(visual_hash_u32(value) >> 8u) * (1.0 / 16777216.0);
}
@compute @workgroup_size(128)
fn advect_visual(@builtin(global_invocation_id) invocation: vec3<u32>) {
  let index = invocation.x;
  if (index >= u32(params.visual.z)) { return; }
  let guide = guides[index / VISUAL_SAMPLES_PER_GUIDE];
  let slot = index % VISUAL_SAMPLES_PER_GUIDE;
  let lane = slot % VISUAL_LANES_PER_GUIDE;
  let ordinal = slot / VISUAL_LANES_PER_GUIDE;
  var fine = fine_grains[index];
  let falling = guide.velocity.y < -0.04;
  let emitted = (fine.contact_count & VISUAL_EMITTED) != 0u;
  let lane_seed = guide.id ^ (lane * 0x9e3779b9u);
  let source_angle = visual_hash_unit(lane_seed ^ 0x51a7d39bu) * 6.28318531;
  let source_offset = vec2<f32>(cos(source_angle), sin(source_angle))
    * params.visual.y * 0.15;
  let rest_angle = visual_hash_unit(fine.id ^ 0x51a7d39bu) * 6.28318531;
  let rest_radius = params.visual.y * 0.58
    * sqrt((f32(slot) + 0.5) / f32(VISUAL_SAMPLES_PER_GUIDE));
  let rest_offset = vec2<f32>(cos(rest_angle), sin(rest_angle)) * rest_radius;
  let guide_caught_stream = emitted && guide.contact_count > 0u
    && guide.velocity.y - fine.velocity.y > 0.70;
  if (!falling || guide_caught_stream) {
    // Deposited visual shares return to the one physical guide. A real
    // landing changes their mineral pattern once; rest never re-rolls it.
    if (emitted) {
      fine.id = visual_hash_u32(fine.id ^ (guide.id * 0x9e3779b9u)
        ^ 0x85ebca6bu);
    }
    let settled = guide.contact_count > 0u
      && length(guide.velocity) < 0.10;
    let anchor_shift = length(guide.position - fine.previous_position);
    let locked = !emitted && settled
      && anchor_shift < params.visual.y * 0.35;
    if (!locked) {
      fine.position = guide.position + rest_offset;
      fine.previous_position = guide.position;
    }
    fine.velocity = vec2<f32>(0.0);
    fine.contact_count = 0u;
    fine_grains[index] = fine;
    return;
  }
  let time_step = params.visual.x;
  if (!emitted) {
    // Per-lane cumulative exponential waits: ten events along each narrow
    // ballistic lane, with more separation in time than launch angle.
    if ((fine.contact_count & VISUAL_DORMANT) == 0u) {
      var wait_product = 1.0;
      for (var event = 0u; event <= ordinal; event++) {
        let seed = guide.id ^ (lane * 0x9e3779b9u)
          ^ (event * 0x85ebca6bu);
        let u = max(visual_hash_unit(seed ^ 0x27d4eb2fu), 1.0e-6);
        wait_product *= u;
      }
      let release_wait = -log(max(wait_product, 1.0e-20)) * 0.035;
      fine.velocity = vec2<f32>(release_wait, 0.0);
      fine.contact_count = VISUAL_DORMANT;
    }
    if (fine.velocity.x > time_step) {
      fine.position = guide.position + source_offset;
      fine.previous_position = guide.position;
      fine.velocity.x -= time_step;
      fine_grains[index] = fine;
      return;
    }
    // Box-Muller: every grain in a lane shares one normal launch angle.
    // Every optical share still accounts for only 1/N of its guide's area.
    let u1 = max(visual_hash_unit(lane_seed ^ 0x9e3779b9u), 1.0e-6);
    let u2 = visual_hash_unit(lane_seed ^ 0x85ebca6bu);
    let normal = sqrt(-2.0 * log(u1)) * cos(6.28318531 * u2);
    let angle = clamp(normal * 0.075, -0.24, 0.24);
    let ejection_speed = clamp(abs(guide.velocity.y) * 0.25, 0.12, 0.45);
    fine.position = guide.position + source_offset;
    fine.velocity = guide.velocity
      + ejection_speed * vec2<f32>(sin(angle), -cos(angle));
    fine.previous_position = guide.position;
    fine.contact_count = VISUAL_EMITTED;
    fine_grains[index] = fine;
    return;
  }
  // No spatial halo or fixed lifetime: emitted shares fly until the guide
  // deposits or a contact decelerates it, then return to the Embedding.
  let gravity = params.boundary_pose.z;
  fine.position += fine.velocity * time_step
    + vec2<f32>(0.0, 0.5 * gravity * time_step * time_step);
  fine.velocity.y += gravity * time_step;
  fine_grains[index] = fine;
}
`;
...
export async function createGranularParticleEmbeddingGpu(deviceArgument, canvasArgument,
  ...
  const boundaryBuffer = createBuffer(device, 'VKF authored granular boundary segments',
    ...
    boundaryWords);
  const visualParticleCount = particleCount * GRANULAR_VISUAL_SAMPLES_PER_GUIDE;
  ...
  for (let guide = 0; guide < particleCount; guide += 1) {
    ...
    const phase = ((Math.imul(guide + 1, 0x9e3779b9) >>> 0) / 0x100000000)
      * Math.PI * 2;
    for (let sample = 0; sample < GRANULAR_VISUAL_SAMPLES_PER_GUIDE; sample += 1) {
      const index = guide * GRANULAR_VISUAL_SAMPLES_PER_GUIDE + sample;
      const offset = index * 8;
      const angle = phase + sample * 2.39996323;
      const radius = worldRuntime.policy.grainRadius * 0.72
        * Math.sqrt((sample + 0.5) / GRANULAR_VISUAL_SAMPLES_PER_GUIDE);
  ...
  return Object.freeze({
    ...
    particleCount,
    visualSamplesPerGuide: GRANULAR_VISUAL_SAMPLES_PER_GUIDE,
    visualBuffer,
    render,
    reset,
    resize,
