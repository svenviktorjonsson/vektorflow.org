...
fn swept_wheel_rotation(p:vec2<f32>,v:vec2<f32>,center:vec2<f32>,r:f32,old_angle:f32,delta:f32,elapsed:f32)->SweptMotion{
  ...
  // Use the resolved contact displacement, not endpoint omega at a differently
  // oriented face (which can inject momentum unrelated to the actual sweep).
  let surface=(position-p)/max(elapsed,1.0e-6);
