...
import { createParticleExclusionGpu } from './vf-particle-exclusion-gpu.mjs';
...
export async function createMaterialWorld(device,canvas,compiled,world,arena) {
  ...
  const exclusion=await createParticleExclusionGpu(device,world,physics,physicsSource);
  ...
  return {world,physics,embedding,boundary,exclusion,time:0,accumulator:0,paused:false,revision:0,angle:world.geometry.rotation,
    reset(){this.revision++;physics.reset();this.time=0;this.accumulator=0;this.angle=world.geometry.rotation;
      physics.setWheel({angle:world.geometry.rotation,angularVelocity:0});},
    advance(encoder,elapsed){
      ...
      if(steps){for(let i=0;i<steps;i++){exclusion.begin(encoder);physics.step(encoder,world.time_step);exclusion.audit(encoder,this.angle);}this.accumulator-=steps*world.time_step;this.time+=steps*world.time_step;}
    },destroy(){exclusion.destroy();physics.destroy();embedding.destroy();boundary.destroy();}};
}
...
export async function bootMaterialWorlds(compiled) {
  ...
  function frame(timestamp) {
    ...
    try {
      const app=current();
      const frameAngle=angle;
      const revision=app.revision;
      const retained={time:app.time,accumulator:app.accumulator,angle:app.angle};
      // Sweep transfers the actual frame displacement. A held finger must not
      // leave an old nonzero wall velocity pumping a now-stationary boundary.
      app.physics.setWheel({angle:frameAngle,angularVelocity:0});
      const encoder=device.createCommandEncoder({label:'VKF authored material World frame'});
      app.angle=frameAngle;app.exclusion.beginFrame(encoder);app.exclusion.begin(encoder);
      app.physics.sweepWheel(encoder,Math.max(elapsed,1/120));
      app.exclusion.audit(encoder,frameAngle);
      app.advance(encoder,elapsed);
      app.exclusion.finishFrame(encoder);
      device.queue.submit([encoder.finish()]);pending=true;
      app.exclusion.accepted().then(()=>{
        if(stopped||app!==current()||revision!==app.revision){pending=false;return;}
        ...
        app.boundary.render(display,canvas.getContext('webgpu').getCurrentTexture().createView(),app.physics.policy,app.physics.wheel,frameAngle);
        device.queue.submit([display.finish()]);device.queue.onSubmittedWorkDone().then(()=>{pending=false;},fail);
      },error=>{pending=false;if(revision!==app.revision)return;Object.assign(app,retained);fail(error);});
