...
@compute @workgroup_size(128) fn dense_build(@builtin(global_invocation_id) gid:vec3<u32>){
  ...
  for(var y=-1;y<=1;y++){for(var x=-1;x<=1;x++){let c=cell+vec2<i32>(x,y);if(!motion_valid(c)){continue;}let b=motion_bucket(c);let count=min(atomicLoad(&motion_cells[b]),motion.grid.z);
    for(var slot=0u;slot<count;slot++){let j=atomicLoad(&motion_items[b*motion.grid.z+slot]);if(j<=i){continue;}let other=motion_state(j);let sep=motion_separation(i,j);let d=motion_length(sep);if(d>diameter+reach||d<1.0e-12){continue;}
      ...
      let separation=motion_separation_parts(i,j);let gap=motion_pair_gap_parts(separation);let n=sep/d;
      // Use the same compensated support plane as the primal velocity slack.
      // A rounded unit normal cannot safely reuse an unscaled radial gap.
      // D-diameter-epsilon*D >= reserve is exactly D >= this padded radius.
      let epsilon=0.00000095367431640625;let buffer=(epsilon*diameter+motion_solve_reserve())/(1.0-epsilon);
      let gamma=-motion_plane_gap_parts(separation,n,-diameter,buffer)/h;
      ...
      let mu=select(0.0,motion.material.y,d<=diameter+motion.material.w*8.0);dense_add(i,j,n,gamma,mu,vec2<f32>(0.0),0u);
    }
  }}
