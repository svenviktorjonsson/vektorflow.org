...
export async function createPreventiveParticleContactGpu(device,world,physics,source,resources){
  ...
  const compileEntries=entries.filter(name=>(world.kind!=='liquid'||!/^contact_(project|repair)_\d+$/.test(name))&&(!componentBarrier||!globalPrimalEntries.has(name))&&(!name.startsWith('motion_twist_')||requestedSolve==='twist'||requestedSolve==='adaptive'||name==='motion_twist_begin'&&['affine','dual','mprgp','accelerated','blockpivot'].includes(requestedSolve)));
