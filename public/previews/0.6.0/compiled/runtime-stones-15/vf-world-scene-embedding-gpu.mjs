...
@fragment fn material_fragment(v:Out)->@location(0) vec4<f32>{
  ...
  // Derivatives remain outside the material branch. The vast floor does not
  // evaluate the costly granite field at every pixel.
  let footprint=max(length(dpdx(v.local)),length(dpdy(v.local)));
  let stone=scene.flags.x<0.5&&v.stone>=0.0;
  var relief=0.0;var pit=0.0;var pitVisibility=0.0;
  if(stone){
    let seed=hash(u32(v.stone)+8187u);
    let material=granite(v.local,albedo,seed,u32(v.stone),footprint);albedo=material.rgb;roughness=material.a;
    ...
    pitVisibility=1.0-smoothstep(0.004,0.016,footprint);
    ...
    pit=1.0-smoothstep(0.25,0.40,reliefField);
    relief=(reliefField-0.5)*mix(0.0018,0.0028,unit(seed+131u))*pitVisibility;
  }
  ...
  if(stone){
    n=normalize(n-clamp(gradient,vec3<f32>(-0.3),vec3<f32>(0.3)));
    // Recesses have modest ambient occlusion; their light-facing rim casts
    // the much stronger directional shade into the pit.
    cavityAmbient=1.0-0.06*pit*pitVisibility;
    microOcclusion=1.0-0.62*pit*pitVisibility
      *smoothstep(-0.02,0.04,dot(gradient,l));
  }
  let projected=scene.light_vp*vec4<f32>(v.p+n*select(0.002,0.001,scene.flags.x<0.5),1.0);
  ...
  if(all(uv>=vec2<f32>(0.0))&&all(uv<=vec2<f32>(1.0))&&ndc.z>=0.0&&ndc.z<=1.0){
    if(scene.flags.x<0.5){
      // One shadow silhouette. Estimate its signed edge distance in the
      // light's depth map, then apply one distance-scaled smoothstep. Unlike
      // offset PCF silhouettes this cannot make several displaced shadows.
      let dimensions=textureDimensions(shadow_texture);
      let pixel=clamp(vec2<i32>(uv*vec2<f32>(dimensions)),vec2<i32>(0),vec2<i32>(dimensions)-1);
      let centerDepth=textureLoad(shadow_texture,pixel,0);
      let blocked=centerDepth<ndc.z-0.000002;
      var blockerDepth=centerDepth;var blockerFound=blocked;
      var nearestEdge=7.0;
      for(var ring=0;ring<2;ring++){
        let radius=array<i32,2>(1,5)[ring];
        for(var direction=0;direction<4;direction++){
          let axis=array<vec2<i32>,4>(vec2<i32>(-1,0),vec2<i32>(1,0),vec2<i32>(0,-1),vec2<i32>(0,1))[direction];
          let at=clamp(pixel+axis*radius,vec2<i32>(0),vec2<i32>(dimensions)-1);
          let depth=textureLoad(shadow_texture,at,0);
          let sampleBlocked=depth<ndc.z-0.000002;
          if(sampleBlocked!=blocked){nearestEdge=min(nearestEdge,f32(radius)*0.5);}
          if(sampleBlocked&&!blockerFound){blockerDepth=depth;blockerFound=true;}
        }
      }
      if(nearestEdge<7.0&&blockerFound){
        let near=0.1;let far=20.0;
        let receiver=near*far/(far-ndc.z*(far-near));
        let blocker=near*far/(far-blockerDepth*(far-near));
        let gap=max(0.0,receiver-blocker);
        let pixelWidth=2.0*receiver*tan(25.0*3.141592654/180.0)/f32(dimensions.x);
        let width=clamp(0.6+gap*scene.domain_span.w/max(blocker,0.1)/pixelWidth,0.6,5.0);
        let signedDistance=select(nearestEdge,-nearestEdge,blocked);
        shadow=smoothstep(-width,width,signedDistance);
      }else{shadow=select(1.0,0.0,blocked);}
    }else{
      shadow=textureSampleCompareLevel(shadow_texture,shadow_sampler,uv,ndc.z-0.00008);
    }
  }
  ...
  let linear=max(vec3<f32>(0.0),v.emission+albedo*(vec3<f32>(0.09)*cavityAmbient+scene.flags.yzw*irradiance*diffuse*shadow*microOcclusion/3.141592654)+vec3<f32>(sheen*shadow));
