...
export async function createMechanicalWorldGpu(device,world,initial,shaderSource) {
  ...
  return {device,buffers,initial,
    ...
    releaseHeld(omega=[0,0,0]){
      const id=f[24];f[24]=-1;
      if(world.kind!=='rigid'||id<0||id>=initial.bodyCount)return;
      const spin=omega.map(value=>Number.isFinite(value)?value:0),speed=Math.hypot(...spin),scale=Math.min(1,5/Math.max(speed,1e-8));
      device.queue.writeBuffer(buffers[0],id*80+48,new Float32Array([...spin.map(value=>value*scale),0]));
    },
