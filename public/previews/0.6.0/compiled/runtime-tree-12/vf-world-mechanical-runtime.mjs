...
import {treeGenerationParameters,TREE_SPECIES} from '../../live/tree/runtime/vf-tree-live-generation.mjs';

function generateTreeAsset(controls){
  const worker=new Worker(new URL('../../live/tree/runtime/vf-tree-generation-worker.mjs',import.meta.url),{type:'module'});
  return new Promise((resolve,reject)=>{
    worker.onmessage=event=>{worker.terminate();event.data.error?reject(new Error(event.data.error)):resolve(event.data.meshes);};
    worker.onerror=event=>{worker.terminate();reject(new Error(event.message??'Tree generation worker failed'));};
    worker.postMessage(controls);
  });
}
...
export function prepareMechanicalInitialState(world,arenas,asset){
  ...
  if(kind==='rigid'){
    ...
    for(const item of asset){const collisionClass=item.id?.includes('foliage')?3:1;const vertices=convert(item.vertices,-1,(v,d)=>Math.pow(Math.max(0,v[d+2])/8,2)*0.65,(v,d)=>{
      ...
      if(item.id?.includes('wood'))for(let d=0;d<vertices.length;d+=24)vertices[d+15]=item.use_vertex_bark?0:2;
...
export async function bootMaterialWorlds(compiled){
  ...
  const authored=world.kind==='wind'?settings.tree_generation:null;
  if(world.kind==='wind'&&(!authored?.height||!authored?.split_factor||!authored?.turning_factor||!Array.isArray(authored.species)||!Array.isArray(authored.distribution)))throw new Error('VKF tree generation controls are missing');
  const treeControls=world.kind==='wind'?treeGenerationParameters({
    species:query.get('species')??authored.species[0],distribution:query.get('generation')??authored.distribution[1],
    height:query.get('height')??authored.height.value,splitFactor:query.get('split')??authored.split_factor.value,
    turnFactor:query.get('turn')??authored.turning_factor.value,
  }):null;
  await new Promise(resolve=>requestAnimationFrame(resolve));
  const [asset,adapter]=await Promise.all([
    world.kind==='wind'?generateTreeAsset(treeControls):readMechanicalAsset(source.asset),
    navigator.gpu.requestAdapter({powerPreference:'high-performance'}),
  ]);if(!adapter)throw new Error('No WebGPU adapter');const device=await adapter.requestDevice();
  if(treeControls)world.solid_properties.height=treeControls.height;
  ...
  if(settings.wind){const label=document.createElement('label');label.textContent='Wind ';const input=document.createElement('input');input.type='range';input.min='0';input.max='20';input.step='.1';input.value=String(Math.max(0,Math.min(20,world.properties.speed??3.5)));input.setAttribute('aria-label','Wind speed, 0 to 20 metres per second');const output=document.createElement('output');output.value=`${Number(input.value).toFixed(1)} m/s`;input.addEventListener('input',()=>{physics.setSpeed(Number(input.value));output.value=`${Number(input.value).toFixed(1)} m/s`;});label.append(input,output);controls.append(label);}
  if(treeControls){
    const regenerate=(key,value)=>{const url=new URL(location.href);url.searchParams.set(key,value);status.textContent='Regenerating tree geometry…';location.replace(url);};
    const slider=(title,key,value,min,max,step,format)=>{const label=document.createElement('label');label.textContent=`${title} `;const input=document.createElement('input');input.type='range';input.min=String(min);input.max=String(max);input.step=String(step);input.value=String(value);input.setAttribute('aria-label',title);const output=document.createElement('output');output.value=format(value);input.addEventListener('input',()=>{output.value=format(Number(input.value));});input.addEventListener('change',()=>regenerate(key,input.value));label.append(input,output);controls.append(label);};
    const select=(title,key,items,current)=>{const label=document.createElement('label');label.textContent=`${title} `;const input=document.createElement('select');input.setAttribute('aria-label',title);for(const [value,name] of items){const option=document.createElement('option');option.value=value;option.textContent=name;input.append(option);}input.value=current;input.addEventListener('change',()=>regenerate(key,input.value));label.append(input);controls.append(label);};
    slider('Height','height',treeControls.height,authored.height.min,authored.height.max,authored.height.step,value=>`${value.toFixed(2)} m`);
    slider('Split factor','split',treeControls.splitFactor,authored.split_factor.min,authored.split_factor.max,authored.split_factor.step,value=>value.toFixed(2));
    slider('Turning factor','turn',treeControls.turnFactor,authored.turning_factor.min,authored.turning_factor.max,authored.turning_factor.step,value=>value.toFixed(2));
    select('Branch distribution','generation',authored.distribution.map(name=>[name,name]),treeControls.distribution);
    select('Species','species',authored.species.map(name=>[name,TREE_SPECIES[name].name]),treeControls.species);
  }
