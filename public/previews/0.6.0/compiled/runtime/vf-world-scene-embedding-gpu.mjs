...
struct Vertex { @location(0) p:vec3<f32>, @location(1) n:vec3<f32>, @location(2) color:vec4<f32>, @location(3) tag:f32, @location(4) compliance:f32 };
struct Out { @builtin(position) clip:vec4<f32>, @location(0) p:vec3<f32>, @location(1) n:vec3<f32>, @location(2) color:vec4<f32> };
fn rotate(q:vec4<f32>,p:vec3<f32>)->vec3<f32>{return p+2.0*cross(q.xyz,cross(q.xyz,p)+q.w*p);}
fn displacement(p:vec3<f32>)->vec3<f32>{
  ...
  for(var z=0u;z<2u;z++){for(var y=0u;y<2u;y++){for(var x=0u;x<2u;x++){
    ...
    d+=nodes[at.x+scene.grid.x*(at.y+scene.grid.y*at.z)].d.xyz*w.x*w.y*w.z;
  }}}return d;
}
fn posed(v:Vertex)->Out {
  var p=v.p;var n=v.n;
  if(scene.flags.x<0.5&&v.tag>=0.0){let b=bodies[u32(v.tag)];p=b.p.xyz+rotate(b.q,p);n=rotate(b.q,n);}
  if(scene.flags.x>0.5&&v.compliance>0.0){p+=displacement(p)*v.compliance;}
  return Out(scene.vp*vec4<f32>(p,1.0),p,n,v.color);
}
...
fn grass_pose(vertex:u32,id:u32)->Out {
  ...
  let p=root+direction*local.x*width+vec3<f32>(0.0,0.0,height*local.y)+bend*local.y*local.y*0.32;
  ...
  return Out(scene.vp*vec4<f32>(p,1.0),p,n,vec4<f32>(color,1.0));
}
...
@fragment fn material_fragment(v:Out)->@location(0) vec4<f32>{
  if(v.color.a<0.0){return vec4<f32>(v.color.rgb,1.0);}
  let n=normalize(v.n);let l=normalize(scene.sun.xyz-v.p);let projected=scene.light_vp*vec4<f32>(v.p+n*0.012,1.0);
  let ndc=projected.xyz/projected.w;let uv=ndc.xy*vec2<f32>(0.5,-0.5)+0.5;
  var shadow=1.0;
  if(all(uv>=vec2<f32>(0.0))&&all(uv<=vec2<f32>(1.0))&&ndc.z>=0.0&&ndc.z<=1.0){shadow=textureSampleCompareLevel(shadow_texture,shadow_sampler,uv,ndc.z-0.0015);}
  let diffuse=abs(dot(n,l));let view=normalize(scene.eye.xyz-v.p);let sheen=pow(max(0.0,dot(n,normalize(l+view))),24.0)*0.04;
  return vec4<f32>(v.color.rgb*(0.34+0.73*diffuse*shadow)+vec3<f32>(sheen*shadow),1.0);
}
...
@vertex fn parcel_vertex(@builtin(vertex_index) v:u32,@builtin(instance_index) i:u32)->ParcelOut{
  ...
  let location=p.p.xyz+(scene.right.xyz*q.x+scene.up.xyz*q.y)*0.009;
...
function perspective(fov,aspect){const f=1/Math.tan(fov*Math.PI/360),near=.03,far=100;return [f/aspect,0,0,0,0,f,0,0,0,0,far/(near-far),-1,0,0,far*near/(near-far),0];}
...
export async function createWorldSceneEmbeddingGpu(device,canvas,world,physics,meshes,shaderSource){
  ...
  const shadow=device.createTexture({size:{width:1024,height:1024,depthOrArrayLayers:1},format:'depth32float',usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.TEXTURE_BINDING});
  ...
  const attributes=[{shaderLocation:0,offset:0,format:'float32x3'},{shaderLocation:1,offset:12,format:'float32x3'},{shaderLocation:2,offset:24,format:'float32x4'},{shaderLocation:3,offset:40,format:'float32'},{shaderLocation:4,offset:44,format:'float32'}];
  const make=(entry,shadowPass=false,particles=false)=>device.createRenderPipelineAsync({layout:device.createPipelineLayout({bindGroupLayouts:[shadowPass?shadowLayout:layout]}),vertex:{module,entryPoint:entry,buffers:entry.startsWith('mesh')||entry==='shadow_vertex'?[{arrayStride:48,attributes}]:[]},...(shadowPass?{}:{fragment:{module,entryPoint:particles?'parcel_fragment':'material_fragment',targets:[{format,...(particles?{blend:{color:{srcFactor:'src-alpha',dstFactor:'one-minus-src-alpha'},alpha:{srcFactor:'one',dstFactor:'one-minus-src-alpha'}}}:{})}]}}),primitive:{topology:particles?'triangle-strip':'triangle-list',cullMode:'none'},depthStencil:{format:'depth32float',depthWriteEnabled:!particles,depthCompare:'less-equal'}});
  ...
  return {render(encoder,camera,{grass=true,particles=false}={}){
    ...
    const sun=p.sun??(world.kind==='wind'?[-8,-9,14]:[-3,-5,8]);const target=world.kind==='wind'?[0,0,3]:[0,0,.7];const radius=world.kind==='wind'?11:5;
    const ortho=[1/radius,0,0,0,0,1/radius,0,0,0,0,-1/50,0,0,0,0,1];
    f.set(multiply(perspective(camera.fov,w/h),view(camera.pos,camera.target)),0);f.set(multiply(ortho,view(sun,target)),16);
    f.set([...camera.pos,1],32);f.set([...sun,1],36);f.set([...(p.domain_min??[-6,-6,0]),0],40);f.set([...(p.domain_span??[12,12,9]),0],44);u.set([...(p.grid??[12,12,18]),0],48);f.set([world.kind==='wind'?1:0,0,0,0],52);
