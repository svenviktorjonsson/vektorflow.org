...
fn elastic_step(@builtin(global_invocation_id) gid:vec3<u32>){
  ...
  let k=params.material.z/compliance;
  ...
  coupling=(coupling-neighbors*prior_nodes[i].d.xyz)*k*0.3;
  let a=(force+coupling-k*nodes[i].d.xyz-params.material.w*nodes[i].v.xyz)/params.wind.z;
  let v=(nodes[i].v.xyz+a*dt)/(1.0+dt*2.0);
  nodes[i].v=vec4<f32>(v,0.0);nodes[i].d=vec4<f32>(clamp(nodes[i].d.xyz+v*dt,vec3<f32>(-0.5),vec3<f32>(0.5)),0.0);
}
`;
