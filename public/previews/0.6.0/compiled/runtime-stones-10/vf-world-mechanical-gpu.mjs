    ...
    for(var i=0u;i<params.counts.x;i++){
      ...
      for(var j=i+1u;j<params.counts.x;j++){
        ...
        if(vn<0.0){
          ...
          if(im>0.0){bodies[i].w=vec4<f32>(bodies[i].w.xyz+angular_impulse/bodies[i].info.y,bodies[i].w.w);}if(jm>0.0){bodies[j].w=vec4<f32>(bodies[j].w.xyz-angular_impulse/bodies[j].info.y,bodies[j].w.w);}
        }
