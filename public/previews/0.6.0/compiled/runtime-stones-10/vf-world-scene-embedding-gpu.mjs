...
fn granite(p:vec3<f32>,tint:vec3<f32>,seed:u32,kind:u32,footprint:f32)->vec4<f32>{
  ...
  if(kind==1u){
    ...
    color=mix(vec3<f32>(0.61,0.60,0.55)*(0.88+fine*0.18),vec3<f32>(0.82,0.79,0.70),vein*0.72);roughness=0.67;
  }else if(kind==4u){
    // Gneiss: coherent light/dark foliation in object space, not painted UV bands.
    let warp=mineral_noise(p*3.7,seed+503u)-0.5;
    let band=0.5+0.5*sin(dot(p,vec3<f32>(7.5,3.2,5.1))+warp*3.2);
    let light=smoothstep(0.43,0.67,band);
