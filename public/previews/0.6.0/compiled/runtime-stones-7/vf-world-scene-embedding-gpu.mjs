...
fn posed(v:Vertex)->Out {
  ...
  if(scene.flags.x>0.5&&v.compliance>0.0){
    if(v.leaf>0.5){let offset=p-v.anchor;let angle=dot(mode_displacement(v.anchor,1u),v.n);
      let axis=normalize(cross(v.hinge,v.n));let along=clamp(dot(offset,axis)/max(v.length,0.02),0.0,1.0);
      let elastic_angle=angle*smoothstep(0.0,1.0,along);
      p=v.anchor+hinge_rotate(offset,v.hinge,elastic_angle)+displacement(v.anchor)*v.compliance;n=hinge_rotate(n,v.hinge,elastic_angle);
...
@fragment fn material_fragment(v:Out)->@location(0) vec4<f32>{
  ...
  let diffuse=max(0.0,dot(n,l));let view=normalize(scene.eye.xyz-v.p);let sheen=pow(max(0.0,dot(n,normalize(l+view))),mix(90.0,16.0,roughness))*mix(0.13,0.025,roughness);
  let irradiance=scene.sun.w/max(dot(scene.sun.xyz-v.p,scene.sun.xyz-v.p),0.01);
  return vec4<f32>(v.emission+albedo*(vec3<f32>(0.18)+scene.flags.yzw*irradiance*diffuse*shadow)+vec3<f32>(sheen*shadow),1.0);
}
