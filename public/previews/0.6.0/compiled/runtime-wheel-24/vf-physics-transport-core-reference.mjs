...
const PERCEPTUAL_CHANNELS = new Set([
  'color', 'texture', 'mesh', 'sound', 'opacity', 'sprite', 'particleEffect',
]);
...
export function hashPhysicsStateBufferReference(state) {
  let hash = FNV_OFFSET;
  const physicalChannels = state.channels
    ? Object.keys(state.channels).sort().map((name) => state.channels[name]) : [];
  for (const values of [state.positions, state.velocities, ...physicalChannels]) {
...
export function createPhysicsStateBufferReference({ kind, seed = 0, count, precision = 'f32',
  ...
  conserved = null, adapter = 'unspecified', channels = {} }) {
  ...
  if (!channels || typeof channels !== 'object' || Array.isArray(channels)) {
    throw new TypeError('Physics state buffer channels must be a named record');
  }
  const channelSchema = {}; const channelVectors = {}; const channelConservation = {};
  for (const name of Object.keys(channels).sort()) {
    if (!/^[A-Za-z][A-Za-z0-9_]*$/.test(name)) {
      throw new TypeError('Physics state buffer channel names must be identifiers');
    }
    if (PERCEPTUAL_CHANNELS.has(name)) {
      throw new TypeError(`Physics state buffer channel ${name} belongs in an Embedding`);
    }
    const descriptor = channels[name];
    if (!descriptor || typeof descriptor !== 'object' || Array.isArray(descriptor)) {
      throw new TypeError(`Physics state buffer channel ${name} must be a descriptor`);
    }
    const components = descriptor.components ?? 1;
    if (!Number.isInteger(components) || components <= 0) {
      throw new RangeError(`Physics state buffer channel ${name} components must be positive`);
    }
    const conservation = descriptor.conservation ?? 'intensive';
    if (!['intensive', 'extensive'].includes(conservation)) {
      throw new RangeError(`Physics state buffer channel ${name} conservation must be intensive or extensive`);
    }
    if (conservation === 'extensive' && components !== 1) {
      throw new RangeError(`Physics state buffer extensive channel ${name} must be scalar`);
    }
    const fallback = descriptor.default ?? 0;
    const defaults = Array.isArray(fallback) || ArrayBuffer.isView(fallback)
      ? Array.from(fallback) : new Array(components).fill(fallback);
    if (defaults.length !== components || !defaults.every(Number.isFinite)) {
      throw new TypeError(`Physics state buffer channel ${name} default must match its components`);
    }
    const values = descriptor.values ?? Array.from({ length: count * components },
      (_, index) => defaults[index % components]);
    if ((!Array.isArray(values) && !ArrayBuffer.isView(values))
        || values.length !== count * components || !Array.from(values).every(Number.isFinite)) {
      throw new TypeError(`Physics state buffer channel ${name} values must align with the entity axis`);
    }
    if (conservation === 'extensive' && Array.from(values).some((value) => value < 0)) {
      throw new RangeError(`Physics state buffer extensive channel ${name} must be nonnegative`);
    }
    const vector = new ArrayType(values);
    channelSchema[name] = Object.freeze({ components, conservation,
      default: Object.freeze(defaults) });
    channelVectors[name] = vector;
    if (conservation === 'extensive') {
      const total = Array.from(vector).reduce((sum, value) => sum + value, 0);
      channelConservation[name] = { initial: total, inflow: 0, outflow: 0,
        total, error: 0 };
    }
  }
  const perElement = conserved?.perElement ?? 0;
  const state = { kind, seed: seed >>> 0, count, positions, velocities,
    affineComponents, channelSchema: Object.freeze(channelSchema), channels: channelVectors,
    channelConservation,
  ...
  state.vectorBytes = positions.byteLength + velocities.byteLength
    ...
    + Object.values(channelVectors).reduce((bytes, values) => bytes + values.byteLength, 0);
...
export function snapshotPhysicsStateBufferReference(state) {
  return Object.freeze({ positions: state.positions.slice(), velocities: state.velocities.slice(),
    ...
    channels: Object.freeze(Object.fromEntries(Object.entries(state.channels ?? {})
      .map(([name, values]) => [name, values.slice()]))),
    channelConservation: Object.freeze(Object.fromEntries(
      Object.entries(state.channelConservation ?? {})
        .map(([name, receipt]) => [name, Object.freeze({ ...receipt })]))),
...
export function restorePhysicsStateBufferReference(state, snapshot) {
  ...
  if (snapshot.channels) state.channels = Object.fromEntries(Object.entries(snapshot.channels)
    .map(([name, values]) => [name, values.slice()]));
  if (snapshot.channelConservation) state.channelConservation = Object.fromEntries(
    Object.entries(snapshot.channelConservation)
  ...
  state.vectorBytes = state.positions.byteLength + state.velocities.byteLength
    ...
    + Object.values(state.channels ?? {}).reduce((bytes, values) => bytes + values.byteLength, 0);
  state.stateHash = hashPhysicsStateBufferReference(state);
  return state;
}
...
const scalarChannelTotal = (values) => Array.from(values)
  .reduce((sum, value) => sum + value, 0);

const updatePhysicalChannelAccounting = (state, previousChannels, addedCount) => {
  for (const [name, receipt] of Object.entries(state.channelConservation ?? {})) {
    const previous = previousChannels[name]; const current = state.channels[name];
    const previousTotal = scalarChannelTotal(previous);
    const currentTotal = scalarChannelTotal(current);
    if (addedCount > 0) {
      const appended = current.subarray(current.length - addedCount);
      receipt.inflow += scalarChannelTotal(appended);
    } else receipt.outflow += previousTotal - currentTotal;
    receipt.total = currentTotal;
    receipt.error = Math.abs(receipt.initial + receipt.inflow - receipt.outflow - receipt.total);
  }
};
...
export function appendPhysicsStateBufferReference(state,
  { positions, velocities, affine = null, ids = null, channels = {} }) {
  ...
  for (const name of Object.keys(channels)) {
    if (!Object.hasOwn(state.channelSchema ?? {}, name)) {
      throw new TypeError(`state does not own physical channel ${name}`);
    }
  }
  const appendedChannels = {};
  for (const [name, descriptor] of Object.entries(state.channelSchema ?? {})) {
    const supplied = channels[name] ?? Array.from({ length: addedCount * descriptor.components },
      (_, index) => descriptor.default[index % descriptor.components]);
    requireAlignedVector(`appended physical channel ${name}`, supplied,
      descriptor.components, addedCount);
    if (descriptor.conservation === 'extensive'
        && Array.from(supplied).some((value) => value < 0)) {
      throw new RangeError(`appended extensive channel ${name} must be nonnegative`);
    }
    appendedChannels[name] = supplied;
  }
  const previousCount = state.count;
  const previousChannels = Object.fromEntries(Object.entries(state.channels ?? {})
  ...
  for (const [name, values] of Object.entries(appendedChannels)) {
    state.channels[name] = replaceWithAppended(state.channels[name], values);
  }
  state.count += addedCount;
  updatePhysicalChannelAccounting(state, previousChannels, addedCount);
...
export function selectPhysicsStateBufferReference(state, retainedIndices) {
  ...
  for (const [name, descriptor] of Object.entries(state.channelSchema ?? {})) {
    state.channels[name] = select(state.channels[name], descriptor.components);
  }
  state.count = count;
  const removedCount = previousCount - count;
  updatePhysicalChannelAccounting(state, previousChannels, 0);
