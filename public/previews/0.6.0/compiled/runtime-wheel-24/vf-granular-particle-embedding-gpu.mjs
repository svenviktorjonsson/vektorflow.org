...
fn grain_fragment(input: GrainOut) -> @location(0) vec4<f32> {
  ...
  if (!particle_mode) {
    ...
    let settled = 1.0 - smoothstep(0.06, 0.18, length(input.velocity));
    coverage *= mix(1.0, mix(1.0, 0.05, packed), settled);
  }
...
fn advect_visual(@builtin(global_invocation_id) invocation: vec3<u32>) {
  ...
  let spread = (0.17 + 0.10 * min(length(guide.velocity), 2.0))
  ...
  let settled = guide.contact_count > 0u && length(guide.velocity) < 0.06;
  let velocity = select(mix(flow_velocity, individual_velocity, airborne),
    vec2<f32>(0.0), settled);
