...
const PRIMAL_BLOCK_PRECONDITIONER=false;
...
fn primal_block_inverse(diagonal:vec2<f32>,off:f32)->vec4<f32>{
 ...
 let large=max(1.0,(mean+radius)*scale);let small=max(1.0,(mean-radius)*scale);
...
fn primal_slack(owner:u32,row:vec4<u32>,a:u32)->f32{
 var relative=primal_velocity(owner);if(row.x!=0xffffffffu){relative-=primal_velocity(row.x);}
 return dot(bitcast<vec2<f32>>(row.yz),relative)-primal_required(owner,row,a);
}
fn primal_endpoint(i:u32)->vec4<f32>{let p=primal_data(i,7u);return motion_offset_parts(motion_compensated(p.xy,p.zw,primal_velocity(i)*motion_value(8)));}
...
@compute @workgroup_size(1) fn primal_cool(){motion_store(47,max(select(0.000001,1.0e-30,motion.grid.w==4u),motion_value(47)*0.1));}
...
fn primal_weights_one(i:u32){
 ...
 for(var k=0u;k<count;k++){let a=dense_row(i,k);let row=primal_row(i,k);let slack=primal_slack(i,row,a);if(slack<=0.0){atomicOr(&motion_control[6],1024u);return;}primal_put(i,10u+2u*k,vec4<f32>(mu/(slack*slack),0.0,0.0,0.0));}
}
fn primal_gradient_one(i:u32){
 ...
 let inverse=primal_block_inverse(diagonal,off);primal_put(i,90u,inverse);
...
@compute @workgroup_size(1) fn primal_alpha(){
 let total=dense_totals();if(atomicLoad(&motion_control[55])==0u){motion_store(54,total.x);atomicStore(&motion_control[55],1u);}
 let working=total.x>max(1.0e-18,motion_value(54)*0.00001)&&total.y>1.0e-24;
...
fn primal_line_bound_one(i:u32){
 ...
 for(var k=0u;k<count;k++){let a=dense_row(i,k);let row=primal_row(i,k);var relative=d;if(row.x!=0xffffffffu){relative-=primal_data(row.x,2u).zw;}let closing=-dot(bitcast<vec2<f32>>(row.yz),relative);if(closing>0.0){fraction=min(fraction,0.95*primal_slack(i,row,a)/closing);}}
...
fn primal_trial_validate_one(i:u32){
 if(i>=motion.counts.x||motion_value(0)==0.0){return;}let v=primal_data(i,4u).xy;let current=primal_data(i,7u);let endpoint=motion_offset_parts(motion_compensated(current.xy,current.zw,v*motion_value(8)));var safe=motion_rim_gap_parts(endpoint)>motion_solve_reserve();let count=primal_counts(i).x;
 for(var k=0u;k<count;k++){let a=dense_row(i,k);let row=primal_row(i,k);var relative=v;if(row.x!=0xffffffffu){relative-=primal_data(row.x,4u).xy;}safe=safe&&dot(bitcast<vec2<f32>>(row.yz),relative)-primal_required(i,row,a)>0.0;}
...
fn primal_energy_one(i:u32)->vec2<f32>{
 ...
 for(var k=0u;k<count;k++){
  let a=dense_row(i,k);let row=primal_row(i,k);var relative=trial;if(row.x!=0xffffffffu){relative-=primal_data(row.x,4u).xy;}
  let slack=dot(bitcast<vec2<f32>>(row.yz),relative)-primal_required(i,row,a);
  energy-=mu*log(max(vec2<f32>(primal_slack(i,row,a),slack),vec2<f32>(1.0e-30)));
 }
 let current=primal_data(i,7u);let endpoint=motion_offset_parts(motion_compensated(current.xy,current.zw,trial*motion_value(8)));
...
fn primal_cached_operator_one(i:u32)->vec2<f32>{
 ...
 for(var k=0u;k<primal_counts(i).y;k++){let id=primal_incident(i,k);let owner=id/32u;let row=primal_row(owner,id%32u);let n=bitcast<vec2<f32>>(row.yz);let j=select(owner,row.x,owner==i);var other=vec2<f32>(0.0);if(j!=0xffffffffu){other=primal_cache[j];}ap+=n*primal_weight(owner,id%32u)*dot(n,p-other);}
...
fn primal_cached_update_one(i:u32)->f32{
 let residual=primal_data(i,1u).zw-primal_cached_control.alpha*primal_data(i,3u).xy;let z=residual/primal_data(i,0u).zw;let d=primal_data(i,2u).zw+primal_cached_control.alpha*primal_cache[i];primal_put(i,1u,vec4<f32>(primal_cache[i],residual));primal_put(i,2u,vec4<f32>(z,d));return dot(residual,z);
}
@compute @workgroup_size(128) fn primal_cached_component(@builtin(local_invocation_index) lane:u32){
 ...
 for(var outer=0u;outer<3u;outer++){
  ...
  if(lane==0u){motion_store(47,max(select(0.000001,1.0e-30,motion.grid.w==4u),motion_value(47)*0.1));}storageBarrier();
 }
}
