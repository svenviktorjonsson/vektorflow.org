...
fn vertex_main(@builtin(vertex_index) index: u32) -> Out {
  ...
  let world = params.wheel.xy + corner * (params.wheel.z + 0.03);
  let clip = (world - params.view.xy) / (params.view.zw - params.view.xy) * 2.0 - 1.0;
  var output: Out;
...
fn fragment_main(input: Out) -> @location(0) vec4<f32> {
  ...
  let relative = input.world - params.wheel.xy;
  let c = cos(angle); let s = sin(angle);
  let local_point = vec2<f32>(c * relative.x + s * relative.y,
    -s * relative.x + c * relative.y);
  let radial = length(local_point);
  var distance = abs(radial - radius);
  for (var segment = 0u; segment < RIGID_SEGMENT_COUNT; segment = segment + 1u) {
    let local = baffle(segment);
    distance = min(distance, segment_distance(local_point, local.xy, local.zw));
  }
...
export async function createWheelEmbeddingGpu(device, canvas, format, options = {}) {
  const context = canvas.getContext('webgpu');
  const module = device.createShaderModule({ label: 'VKF rigid wheel embedding', code: options.shaderSource ?? WHEEL_WGSL });
  ...
  const worldToScreen = (point, policy) => {
    const view = viewFor(policy, canvas.width, canvas.height);
    return [(point[0]-view[0])/(view[2]-view[0])*canvas.width,
      (view[3]-point[1])/(view[3]-view[1])*canvas.height];
  };
  return Object.freeze({ render, screenToWorld, worldToScreen,
