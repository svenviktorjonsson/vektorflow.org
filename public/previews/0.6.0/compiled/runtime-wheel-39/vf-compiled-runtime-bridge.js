(function(root, factory) {
  ...
  function instantiateWasmRuntime(options) {
    ...
    if (!options.instance && !(bytes instanceof Uint8Array) && !(bytes instanceof ArrayBuffer)) {
    ...
    if (!instance) {
      module = new WebAssembly.Module(bytes instanceof Uint8Array ? bytes : new Uint8Array(bytes));
    ...
    var worldProgramCache = null;

    function worldProgram() {
      if (worldProgramCache !== null) return worldProgramCache;
      for (var i = 0; i < bindingMetas.length; i += 1) {
        if (bindingMetas[i].name === "$world$program") {
          worldProgramCache = JSON.parse(readBindingValue(dataView(), exportsObject, bindingMetas[i]));
          return worldProgramCache;
        }
      }
      return null;
    }

    function worldLayerViews() {
      var program = worldProgram();
      if (!program) return [];
      return program.layers.map(function(layer) {
        var meta = bindingMetas.find(function(binding) { return binding.name === layer.state_binding; });
        if (!meta) throw new Error("compiled World layer arena is unavailable");
        var ptr = Number(exportsObject[meta.ptr_export]());
        var length = Number(exportsObject[meta.len_export]());
        if (length !== layer.count * layer.dimension * 3) throw new Error("compiled World layer arena shape mismatch");
        var particleChannels = {};
        Object.keys(layer.particle_channels || {}).forEach(function(name) {
          var channel = layer.particle_channels[name];
          var channelMeta = bindingMetas.find(function(binding) { return binding.name === channel.binding; });
          if (!channelMeta) throw new Error("compiled Particle channel is unavailable: " + name);
          var channelPtr = Number(exportsObject[channelMeta.ptr_export]());
          var channelLength = Number(exportsObject[channelMeta.len_export]());
          if (channelLength !== layer.count * channel.components)
            throw new Error("compiled Particle channel shape mismatch: " + name);
          particleChannels[name] = new Float64Array(exportsObject.memory.buffer, channelPtr, channelLength);
        });
        return { layer: layer, state: new Float64Array(exportsObject.memory.buffer, ptr, length),
          particleChannels: particleChannels,
          componentStride: 3, positionOffset: 0, velocityOffset: 1, accelerationOffset: 2 };
      });
    }
    ...
    return {
      ...
      // Borrowed dense WASM views: renderers upload these without reparsing
      // JSON or running any physics. Reacquire after memory growth.
      worldLayerViews: worldLayerViews,
      stepWorld: function(id) {
        var update = exportsObject['vkf_world_' + id + '_update'];
        if (typeof update !== 'function') throw new Error('compiled World update is unavailable: ' + id);
        update();
      },
      init: function() {
  ...
  async function instantiateWasmRuntimeAsync(options) {
    options = options || {};
    if (!(options.bytes instanceof Uint8Array) && !(options.bytes instanceof ArrayBuffer)) {
      throw new Error('instantiateWasmRuntimeAsync requires wasm bytes');
    }
    var result = await WebAssembly.instantiate(options.bytes, options.imports || {});
    return instantiateWasmRuntime({instance:result.instance,module:result.module,manifest:options.manifest});
  }

  return {
    instantiateWasmRuntime: instantiateWasmRuntime,
    instantiateWasmRuntimeAsync: instantiateWasmRuntimeAsync,
