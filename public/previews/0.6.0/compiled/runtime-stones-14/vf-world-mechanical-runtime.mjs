...
export async function bootMaterialWorlds(compiled){
  ...
  canvas.addEventListener('pointerdown',event=>{event.preventDefault();if(event.button!==0)return;
    ...
    if(world.kind==='rigid'){
      ...
      if(hit){drag={kind:'stone',id:event.pointerId,body:choice.i,y:event.clientY,z:snapshot[choice.i*20+2],depth:choice.p.depth,pickX:event.clientX-choice.p.x,pickY:event.clientY-choice.p.y,moveY:event.clientY,moveAt:performance.now(),vz:0};physics.setHeld(choice.i,drag.z);}
  ...
  canvas.addEventListener('pointermove',event=>{event.preventDefault();
    ...
    if(drag.kind==='stone'){
      const metres=2*drag.depth*Math.tan(camera.fov*Math.PI/360)/canvas.getBoundingClientRect().height,now=performance.now(),dt=Math.max(.008,(now-drag.moveAt)/1000);
      drag.vz=0.35*drag.vz+0.65*(drag.moveY-event.clientY)*metres/dt;drag.moveY=event.clientY;drag.moveAt=now;
      physics.setHeld(drag.body,Math.max(drag.z,Math.min(initial.sceneFrame.liftCeiling,drag.z+(drag.y-event.clientY)*metres)));
    }
  ...
  const release=event=>{event.preventDefault();pointers.delete(event.pointerId);pinchDistance=0;if(drag?.id===event.pointerId){
    if(drag.kind==='stone'){
      const metres=2*drag.depth*Math.tan(camera.fov*Math.PI/360)/canvas.getBoundingClientRect().height;
      const forward=sub(camera.target,camera.pos),length=Math.hypot(...forward),direction=forward.map(value=>value/length);
      const right=[direction[1],-direction[0],0],rightLength=Math.hypot(...right),r=right.map(value=>value/Math.max(rightLength,1e-8));
      const up=[r[1]*direction[2]-r[2]*direction[1],r[2]*direction[0]-r[0]*direction[2],r[0]*direction[1]-r[1]*direction[0]];
      const grab=r.map((value,i)=>value*drag.pickX*metres-up[i]*drag.pickY*metres);
      const body=drag.body*20,mass=snapshot[body+16],inertia=snapshot[body+17],active=performance.now()-drag.moveAt<120;
      const vz=active?drag.vz:0,denominator=grab.reduce((sum,value)=>sum+value*value,0)+inertia/mass;
      physics.releaseHeld([grab[1]*vz/denominator,-grab[0]*vz/denominator,0]);
    }drag=null;
  }if(settings.orbit&&pointers.size===1){const remaining=[...pointers.entries()][0];drag={kind:'orbit',id:remaining[0],x:remaining[1][0],y:remaining[1][1]};}if(canvas.hasPointerCapture(event.pointerId))canvas.releasePointerCapture(event.pointerId);report();};canvas.addEventListener('pointerup',release);canvas.addEventListener('pointercancel',release);canvas.addEventListener('lostpointercapture',event=>{pointers.delete(event.pointerId);if(drag?.id===event.pointerId){if(drag.kind==='stone')physics.releaseHeld();drag=null;}});
