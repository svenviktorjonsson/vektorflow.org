...
@fragment fn material_fragment(v:Out)->@location(0) vec4<f32>{
  ...
  if(all(uv>=vec2<f32>(0.0))&&all(uv<=vec2<f32>(1.0))&&ndc.z>=0.0&&ndc.z<=1.0){
    if(scene.flags.x<0.5){
      // PCSS: find actual blockers in the one light's perspective depth map,
      // then widen the filter by blocker-to-receiver distance. Contact stays
      // sharp; lifted rock shadows soften. No body-shaped dark overlay.
      ...
      var blockerDepth=0.0;var blockers=0.0;
      for(var sample=0;sample<5;sample++){
        let offset=array<vec2<i32>,5>(vec2<i32>(0,0),vec2<i32>(-8,0),vec2<i32>(8,0),vec2<i32>(0,-8),vec2<i32>(0,8))[sample];
        let at=clamp(pixel+offset,vec2<i32>(0),vec2<i32>(dimensions)-1);
        let depth=textureLoad(shadow_texture,at,0);
        if(depth<ndc.z-0.000002){blockerDepth+=depth;blockers+=1.0;}
      }
      if(blockers>0.0){
        let near=0.1;let far=20.0;
        let receiver=near*far/(far-ndc.z*(far-near));
        let blocker=near*far/(far-(blockerDepth/blockers)*(far-near));
        let gap=max(0.0,receiver-blocker);
        let pixelWidth=2.0*receiver*tan(25.0*3.141592654/180.0)/f32(dimensions.x);
        let radius=clamp(0.6+gap*scene.domain_span.w/max(blocker,0.1)/pixelWidth,0.6,8.0);
        shadow=0.0;
        for(var y=-1;y<=1;y++){for(var x=-1;x<=1;x++){
          let offset=vec2<f32>(f32(x),f32(y))*radius*0.55/vec2<f32>(dimensions);
          shadow+=textureSampleCompareLevel(shadow_texture,shadow_sampler,uv+offset,ndc.z-0.000002)/9.0;
        }}
      }
    }else{
