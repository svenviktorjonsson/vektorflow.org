  ...
  if(options.preventiveContact){
    ...
    if(options.forceGeometryCaching){
      ...
      for(const name of ['cache_pressure_geometry','cached_divergence_lambda','cached_density_lambda','cached_apply_pressure']){
        const pipeline=await createOptionalGpuPipeline(device,'compute',{label:'VKF optional pressure geometry cache',layout:cachePipelineLayout,compute:{module:shader,entryPoint:`preventive_${name}`}});
