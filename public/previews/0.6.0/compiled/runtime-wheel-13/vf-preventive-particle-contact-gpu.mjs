...
export async function createPreventiveParticleContactGpu(device,world,physics,source,resources){
  ...
  const compileEntries=entries.filter(name=>world.kind!=='liquid'||!/^contact_(project|repair)_\d+$/.test(name));
  const pipelines={};
  for(const entryPoint of compileEntries){
    ...
    const descriptor={label:`VKF contact ${entryPoint}`,layout:selectedLayout,compute:{module,entryPoint:star?'contact_star_project':tile?'contact_tiled_project':entryPoint==='al_affine_component'?'al_component':entryPoint,constants}};
  ...
  let queryIndex=0,started=0,selected='parallel',moving=false,eventBudget=4,fixedBudget=r.rotatingTrajectory?32:null,liquidDispatch=r.rotatingTrajectory&&!sand&&r.count<=1908&&pipelines.primal_cached_component?'cached':'auto',liquidSolve='barrier';
