...
@compute @workgroup_size(128)
fn project_particle_exclusion(@builtin(global_invocation_id) gid:vec3<u32>){
  let i=gid.x;if(i>=params.counts.x){return;}
  let p=particles[i].position;let cell=cell_coordinate(p);let diameter=params.fluid.y*0.92;
  // A small solve margin separates numerical convergence from physical radius.
  let solve_distance=diameter*1.02;var correction=vec2<f32>(0.0);var contacts=0.0;
  for(var y=-1;y<=1;y++){for(var x=-1;x<=1;x++){let at=cell+vec2<i32>(x,y);if(!valid_cell(at)){continue;}let bucket=cell_index(at);let count=min(atomicLoad(&cell_counts[bucket]),params.counts.w);
    for(var slot=0u;slot<count;slot++){let j=atomicLoad(&cell_items[bucket*params.counts.w+slot]);if(j>=params.counts.x||j==i){continue;}
      let delta=p-particles[j].position;let distance=length(delta);
      if(distance<solve_distance){var normal=delta/max(distance,1.0e-12);if(distance<1.0e-10){normal=vec2<f32>(select(-1.0,1.0,i>j),0.0);}
        correction+=normal*(solve_distance-distance)*0.5;contacts+=1.0;
      }
    }
  }}
  let destination=p+correction/max(1.0,contacts*0.5);
  particles[i].scratch_velocity=resolve_wheel_motion(p,destination,particles[i].velocity,diameter*0.5).position;
}
@compute @workgroup_size(128)
fn apply_particle_exclusion(@builtin(global_invocation_id) gid:vec3<u32>){if(gid.x<params.counts.x){particles[gid.x].position=particles[gid.x].scratch_velocity;}}
...
export async function createLiquidParticleWorldGpuRuntime(deviceArgument, options = {}) {
  ...
  const entryPoints = ['predict', 'clear_cells', 'fill_cells', 'sort_cells', 'divergence_lambda',
    ...
    'apply_filter', 'update_diffuse_and_render', 'sweep_wheel', 'project_particle_exclusion', 'apply_particle_exclusion'];
  ...
  if(options.preventiveContact){
    ...
    await Promise.all(['predict','divergence_lambda','density_lambda','apply_pressure','classify_and_filter','apply_filter'].map(async name=>{
      preventivePipelines[name]=await device.createComputePipelineAsync({layout:guardedLayout,compute:{module:shader,entryPoint:`preventive_${name}`}});
    }));
    if(options.forceGeometryCaching){
      const bytes=seed.count*(2+9*policy.maximumParticlesPerCell)*16;
      ...
      await Promise.all(['cache_pressure_geometry','cached_divergence_lambda','cached_density_lambda','cached_apply_pressure'].map(async name=>{pressureGeometryPipelines[name]=await device.createComputePipelineAsync({layout:cachePipelineLayout,compute:{module:shader,entryPoint:`preventive_${name}`}});}));
    }
  }
  ...
  const encodeFixedStep = (pass) => {
    ...
    for(let iteration=0;iteration<32;iteration++){
      dispatch(pass,'project_particle_exclusion',seed.count);dispatch(pass,'apply_particle_exclusion',seed.count);buildGrid(pass);
    }
  ...
  const predictForces=(encoder)=>{
    ...
    if(r.forceIndirect){for(const name of ['clear_cells','fill_cells','sort_cells']){pass.setPipeline(pipelines[name]);pass.setBindGroup(0,bindGroup);pass.dispatchWorkgroupsIndirect(r.dispatchArgs,name==='fill_cells'?36:48);}}
    else buildGrid(pass);
    ...
    const pressure=name=>{if(pressureGeometryBuffer)cached(`cached_${name}`);else run(name);};
  ...
  const publishParticles=(encoder)=>{
    ...
    buildGrid(pass);dispatch(pass,'update_diffuse_and_render',policy.diffuseCapacity);pass.end();
  };
  const sweepWheel = (encoder, elapsed) => {
    ...
    const contacts=encoder.beginComputePass({label:'Swept liquid exclusion network'});
    buildGrid(contacts);
    for(let iteration=0;iteration<32;iteration++){
      dispatch(contacts,'project_particle_exclusion',seed.count);dispatch(contacts,'apply_particle_exclusion',seed.count);buildGrid(contacts);
    }
    contacts.end();
  ...
  return Object.freeze({ kind: 'liquid-particle-world-gpu-runtime:v1', device,
    ...
    gridCellCount, pipelines: Object.freeze(pipelines), step, stepMany, sweepWheel, predictForces, publishParticles,
