...
fn motion_position_parts(i:u32,duration:f32)->vec4<f32>{
 if(motion_ledger()&&atomicLoad(&motion_control[18])!=0u){let s=bitcast<vec4<f32>>(motion_actors[i*128u]);let t=bitcast<f32>(motion_actors[i*128u+1u].z);let low=bitcast<vec4<f32>>(motion_actors[i*128u+122u]).zw;return motion_compensated(s.xy,low,s.zw*(motion_value(13)+duration-t));}
 let s=bitcast<vec4<f32>>(motion_particles[i*motion.counts.y]);let low=bitcast<vec4<f32>>(motion_actors[i*128u+122u]).xy;let velocity=select(s.zw,bitcast<vec4<f32>>(motion_actors[i*128u+125u]).zw,atomicLoad(&motion_control[17])!=0u);return motion_compensated(s.xy,low,velocity*duration);
}
...
fn motion_squared_gap(s:vec2<f32>,radius:f32)->f32{
 ...
 let error=fma(s.x,s.x,-x)+fma(s.y,s.y,-y)-fma(radius,radius,-rr);
...
fn motion_boundary_solve_reserve(gap:f32)->f32{if(CONTACT_PERSISTENT_SKIN){return 2.0*motion.material.w;}return select(motion_solve_reserve(),min(2.0*motion.material.w,max(0.0,gap*0.25)),motion.grid.w==6u);}
...
fn motion_plane_gap_parts(p:vec4<f32>,n:vec2<f32>,radius:f32,buffer:f32)->f32{
 ...
 let low=first.z+second.z+third.z+fma(n.x,p.x,-products.x)+fma(n.y,p.y,-products.y)+dot(n,p.zw)+fma(radius,norm_hi,-radial)-fma(buffer,norm_hi,-padding)+(radius-buffer)*norm_lo;
 return third.x+low;
}
...
fn motion_vector_rotated_parts(parts:vec4<f32>,angle:f32)->vec4<f32>{
 ...
 let error=fma(vec2<f32>(basis.x),parts.xy,-a)+fma(vec2<f32>(basis.y),perpendicular,-b);let sum=motion_two_sum(a,b);
 return motion_two_sum(sum.xy,error+sum.zw+motion_rotate(parts.zw,angle));
}
...
@compute @workgroup_size(1) fn motion_clearance_end(){
 ...
 if(CONTACT_PERSISTENT_SKIN){if(motion_value(48)<motion.material.w){atomicOr(&motion_control[6],512u);return;}motion_store(52,2.0*motion.material.w);motion_store(49,motion.material.w);return;}
...
@compute @workgroup_size(128) fn motion_enter_rotating_frame(@builtin(global_invocation_id) gid:vec3<u32>){
 let i=gid.x;if(i>=motion.counts.x||atomicLoad(&motion_control[57])==0u){return;}let s=motion_state(i);let offset=motion_offset_parts(motion_position_parts(i,0.0));let omega=motion_value(58);let relative=s.zw-omega*vec2<f32>(-offset.y,offset.x);let h=motion_value(8);
 // Rotating-reference integration includes the local Coriolis and centrifugal
 // acceleration. This is an integration coordinate change, not wind/animation.
 let local_acceleration=-2.0*omega*vec2<f32>(-relative.y,relative.x)+omega*omega*offset.xy;
 let mapped=relative+0.5*h*local_acceleration;
...
@compute @workgroup_size(1) fn motion_dispatch_prepare(){let pending=motion_value(0)>0.0&&atomicLoad(&motion_control[6])==0u;let groups=select(0u,(motion.counts.x+127u)/128u,pending);atomicStore(&motion_dispatch_args[0],groups);atomicStore(&motion_dispatch_args[1],1u);atomicStore(&motion_dispatch_args[2],1u);atomicStore(&motion_dispatch_args[3],groups);atomicStore(&motion_dispatch_args[4],1u);atomicStore(&motion_dispatch_args[5],1u);atomicStore(&motion_dispatch_args[6],select(0u,1u,pending));atomicStore(&motion_dispatch_args[7],1u);atomicStore(&motion_dispatch_args[8],1u);}
...
fn motion_prepare(){
  ...
  var horizon=min(motion_value(0),min(motion.minimum.z*1.2/max(motion_value(5)+abs(motion_value(3))*WHEEL_RADIUS,1.0e-12),0.02/max(abs(motion_value(3)),1.0e-12)));
...
@compute @workgroup_size(1) fn motion_bound_proposal_horizon(){
 ...
 let h=min(requested,motion.minimum.z*1.2/max(motion_value(5)+abs(motion_value(3))*WHEEL_RADIUS,1.0e-12));
...
fn motion_find_body(i:u32){
  ...
  if(!ledger||rebuilding||motion_actors[i*128u+1u].y!=0u){
...
fn motion_end_parts(i:u32,duration:f32)->vec4<f32>{let p=motion_position_parts(i,duration);if(atomicLoad(&motion_control[57])==0u){return p;}return motion_rotated_parts(p,motion_value(58)*duration);}
...
@compute @workgroup_size(128) fn motion_guard_end(@builtin(global_invocation_id) gid:vec3<u32>){
  ...
  let own=motion_state(i);let padding=radius+motion.material.w;let q=motion_local_end_position(i,duration);let lo=motion_cell(min(own.xy,q)-padding);let hi=motion_cell(max(own.xy,q)+padding);
...
fn motion_advance_control(){
  if(atomicLoad(&motion_control[6])!=0u||motion_value(0)==0.0){return;}let fraction=motion_value(4);var duration=select(motion_value(8)*select(fraction*0.999,1.0,fraction>=1.0),0.0,atomicLoad(&motion_control[11])!=0u);
  if(atomicLoad(&motion_control[11])!=0u&&fraction>0.0){motion_store(54,motion_value(8)*fraction*0.999);}else if(duration>0.0){motion_store(54,0.0);}
...
` + DENSE_CONTACT_WGSL + CONTACT_PRIMAL_BARRIER_WGSL + CONTACT_DUAL_COMPONENT_CANDIDATES_WGSL + CONTACT_AUGMENTED_VELOCITY_WGSL + CONTACT_TILED_PROJECTION_WGSL + CONTACT_CACHED_STAR_WGSL;
...
export function createPreventiveContactResources(device,world,count){
  ...
  return {uniform,cells,items,control,readback,actors,incoming,heap,dispatchArgs,precisionOutput,primalScratch,primalScratchFields,emptyLayout,emptyGroup,forceLayout,forceGroup,side,minimum,radius,cellWidth,margin,count,reset,destroy(){for(const b of [uniform,cells,items,control,readback,actors,incoming,heap,dispatchArgs,precisionOutput,primalScratch])b.destroy();}};
}

export async function createPreventiveParticleContactGpu(device,world,physics,source,resources){
  if(resources.rotatingTrajectory&&(resources.experimentalSolve!=='barrier'||!source.includes('fn motion_enter_rotating_frame')))throw new Error('Rotating-reference experiment requires its compiled barrier specialization');
  ...
  entries.push('primal_cg_begin','primal_weights','primal_component','primal_finish','primal_energy','primal_energy_validate','motion_exit_frame','motion_exit_frame_end');
  ...
  if(source.includes('fn primal_cached_component'))entries.push('primal_cached_component');
  if(source.includes('fn primal_adaptive_limit'))entries.push('primal_adaptive_limit');
  ...
  const pipelines=Object.fromEntries(await Promise.all(entries.map(async entryPoint=>{
    ...
    return [entryPoint,await device.createComputePipelineAsync({layout:selectedLayout,compute:{module,entryPoint:star?'contact_star_project':tile?'contact_tiled_project':entryPoint==='al_affine_component'?'al_component':entryPoint,constants}})];
  })));
  ...
  const dispatch=(pass,name,count,solver=false)=>{pass.setBindGroup(2,['contact_star_reset','contact_star_compact'].includes(name)?starCompactGroup:name==='motion_export_precision'?exportGroup:parallelDualControllers.has(name)?parallelDualGroup:controllerEntries.has(name)?controllerGroup:group);pass.setPipeline(pipelines[name]);if(name.startsWith('dense_')||name.startsWith('primal_dual_global_')||['primal_operator','primal_update','primal_conjugate'].includes(name))pass.dispatchWorkgroupsIndirect(r.dispatchArgs,solver?12:0);else pass.dispatchWorkgroups(Math.ceil(count/128));};
  const single=(pass,name)=>{pass.setBindGroup(2,['contact_star_reset','contact_star_compact'].includes(name)?starCompactGroup:parallelDualControllers.has(name)?parallelDualGroup:controllerEntries.has(name)?controllerGroup:group);pass.setPipeline(pipelines[name]);pass.dispatchWorkgroups(1);};
  ...
  let queryIndex=0,started=0,selected='parallel',moving=false,eventBudget=4,fixedBudget=r.rotatingTrajectory?32:null,liquidDispatch=r.rotatingTrajectory&&!sand&&r.count<=1908?'cached':'auto',liquidSolve='barrier';
  ...
  return {
    ...
    advance(encoder,{events=8}={}){if(!Number.isInteger(events)||events<1||events>8||queryIndex>126)throw new Error('Contact submission exceeds its bounded event/query budget');if(queryIndex===0)stageLabels.length=0;if(timestamp){const stamp=encoder.beginComputePass({timestampWrites:{querySet:query,beginningOfPassWriteIndex:queryIndex++}});stamp.end();}const open=(label='prepare')=>{const next=stageQuery?profileOpen(encoder,label):encoder.beginComputePass({label:'VKF preventive contact events'});next.setBindGroup(0,empty);next.setBindGroup(1,empty);next.setBindGroup(2,group);return next;};let pass=open();const changeStage=label=>{if(stageQuery){pass.end();pass=open(label);}};dispatch(pass,'motion_pause_velocity',r.count);single(pass,'motion_begin');
      for(let event=0;event<events;event++){
        ...
        if(liquidSolve==='barrier'){single(pass,'motion_frame_begin');dispatch(pass,r.rotatingTrajectory?'motion_enter_rotating_frame':'motion_enter_frame',r.count);}single(pass,'motion_dispatch_prepare');
        single(pass,'motion_reset_speed');dispatch(pass,'motion_measure_speed',r.count);
        dispatch(pass,'motion_clear',r.side*r.side);dispatch(pass,'motion_fill_static',r.count);dispatch(pass,'motion_sort',r.side*r.side);
        ...
        else if(!sand&&r.count<=4096&&liquidDispatch!=='global'){pass.setBindGroup(2,group);pass.setPipeline(liquidDispatch==='cached'?pipelines.primal_cached_component:pipelines.primal_component);pass.dispatchWorkgroupsIndirect(r.dispatchArgs,24);}
        else if(!sand){
        ...
        if(sand)for(let iteration=0;iteration<16;iteration++)for(let color=0;color<9;color++)dispatch(pass,`contact_repair_${color}`,r.side*r.side);
        if(!sand&&!['projected','tiled','star'].includes(liquidSolve))dispatch(pass,'primal_finish',r.count);
