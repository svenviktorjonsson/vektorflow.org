...
fn cache_pressure_geometry_body(index:u32){
 ...
 let material_density=density;var boundary_gradient=vec2<f32>(0.0);
 ...
 pressure_geometry[pressure_geometry_slot(index,2u)]=vec4<u32>(bitcast<u32>(material_density),0u,0u,0u);
}
@compute @workgroup_size(128) fn cache_pressure_geometry(@builtin(global_invocation_id) gid:vec3<u32>){let i=gid.x;if(i<params.counts.x){cache_pressure_geometry_body(i);}}
@compute @workgroup_size(128) fn preventive_cache_pressure_geometry(@builtin(global_invocation_id) gid:vec3<u32>){let i=gid.x;if(i<params.counts.x&&motion_force_ready()){cache_pressure_geometry_body(i);}}
fn cached_pressure_sample(index:u32)->ConstraintSample{
 let fixed=bitcast<vec4<f32>>(pressure_geometry[pressure_geometry_slot(index,0u)]);let boundary=pressure_geometry[pressure_geometry_slot(index,1u)];let material=pressure_geometry[pressure_geometry_slot(index,2u)];var neighbor_velocity=0.0;
 for(var k=0u;k<boundary.w;k++){let row=pressure_geometry[pressure_geometry_slot(index,3u+k)];neighbor_velocity+=dot(bitcast<vec2<f32>>(row.yz),particles[row.x].velocity);}
 let rate=dot(fixed.zw,particles[index].velocity)-neighbor_velocity-bitcast<f32>(boundary.z);
 return ConstraintSample(fixed.x,bitcast<f32>(material.x),rate,fixed.y,boundary.w,fixed.zw);
}
fn cached_divergence_lambda_body(i:u32){let sample=cached_pressure_sample(i);let residual=max(0.0,sample.density_rate-params.material.y);particles[i].lambda=-residual/sample.denominator;particles[i].density=sample.material_density;particles[i].neighbor_count=sample.neighbor_count;}
fn cached_density_lambda_body(i:u32){let sample=cached_pressure_sample(i);let error=sample.density/params.material.x-1.0;let residual=max(0.0,error+params.fluid.x*sample.density_rate-params.material.z)/params.fluid.x;particles[i].lambda=-residual/sample.denominator;particles[i].scratch_velocity.x=particles[i].scratch_velocity.x+particles[i].lambda*params.force.z;particles[i].density=sample.material_density;particles[i].neighbor_count=sample.neighbor_count;}
fn cached_apply_pressure_body(i:u32){
 let own=particles[i].lambda;let inverse_mass=1.0/params.fluid.w;let boundary=pressure_geometry[pressure_geometry_slot(i,1u)];var correction=vec2<f32>(0.0);
 for(var k=0u;k<boundary.w;k++){let row=pressure_geometry[pressure_geometry_slot(i,3u+k)];correction+=inverse_mass*(own+particles[row.x].lambda)*bitcast<vec2<f32>>(row.yz);}
...
@compute @workgroup_size(128) fn cached_apply_pressure(@builtin(global_invocation_id) gid:vec3<u32>){let i=gid.x;if(i<params.counts.x){cached_apply_pressure_body(i);}}
@compute @workgroup_size(128) fn preventive_cached_divergence_lambda(@builtin(global_invocation_id) gid:vec3<u32>){let i=gid.x;if(i<params.counts.x&&motion_force_ready()){cached_divergence_lambda_body(i);}}
@compute @workgroup_size(128) fn preventive_cached_density_lambda(@builtin(global_invocation_id) gid:vec3<u32>){let i=gid.x;if(i<params.counts.x&&motion_force_ready()){cached_density_lambda_body(i);}}
@compute @workgroup_size(128) fn preventive_cached_apply_pressure(@builtin(global_invocation_id) gid:vec3<u32>){let i=gid.x;if(i<params.counts.x&&motion_force_ready()){cached_apply_pressure_body(i);}}
`;
