...
fn pixel_to_world(position: vec2<f32>) -> vec2<f32> {
  let uv = position / params.canvas.xy;
  return vec2<f32>(mix(params.view.x, params.view.z, uv.x),
    mix(params.view.w, params.view.y, uv.y));
}

fn wheel_solid_at(world: vec2<f32>) -> bool {
  if (params.wheel_pose.z < 0.5) { return false; }
  let relative = world - params.wheel.xy;
  if (length(relative) >= params.wheel.z - params.wheel.w) { return true; }
  let c = cos(params.wheel_pose.x);
  let s = sin(params.wheel_pose.x);
  let local = vec2<f32>(c * relative.x + s * relative.y,
    -s * relative.x + c * relative.y);
  for (var segment = 0u; segment < u32(params.wheel_pose.y); segment++) {
    ...
    if (length(local - (bar.xy + edge * along)) <= params.wheel.w) {
      return true;
    }
  }
  return false;
...
fn primary_fragment(input: PrimaryOut) -> @location(0) vec4<f32> {
  ...
  if (wheel_solid_at(pixel_to_world(input.position.xy))) { discard; }
...
export async function createLiquidParticleEmbeddingGpu(deviceArgument, canvasArgument,
  ...
  const updateParams = (time, mode, wheelAngle) => {
    ...
    const wheel = options.wheelGeometry;
    if (wheel) {
  ...
  const render = (encoder, { time = 0, wheelAngle = 0, mode = 'fluid' } = {}) => {
