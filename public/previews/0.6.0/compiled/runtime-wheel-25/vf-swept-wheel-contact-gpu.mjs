...
fn wheel_ray(p:vec2<f32>,d:vec2<f32>,center:vec2<f32>,limit:f32,contact_radius:f32,angle:f32)->SweptHit{
  var best=no_swept_hit();for(var segment=0u;segment < RIGID_SEGMENT_COUNT;segment++){let local=baffle(segment);best=capsule_ray(best,p,d,center+rotate_local(local.xy,angle),center+rotate_local(local.zw,angle),contact_radius);}
...
fn wheel_arc(origin:vec2<f32>,delta:f32,center:vec2<f32>,r:f32,angle:f32)->SweptHit{
  ...
  for(var segment=0u;segment < RIGID_SEGMENT_COUNT;segment++){
...
fn swept_wheel_motion(origin:vec2<f32>,destination:vec2<f32>,velocity:vec2<f32>,center:vec2<f32>,limit:f32,r:f32,angle:f32)->SweptMotion{
  ...
  for(var contact=0u;contact<8u;contact++){
    ...
    if(hit.kind==1u){
      ...
      p=center+rotate_local(outward*limit,rotation);
      // The rim is unilateral, not a circular velocity conveyor. Remove only
      // velocity exiting the final contact; never rotate velocity with the wall.
      let final_normal=-normalize(p-center);
      v-=final_normal*min(0.0,dot(v,final_normal));
...
fn swept_wheel_rotation(p:vec2<f32>,v:vec2<f32>,center:vec2<f32>,r:f32,old_angle:f32,delta:f32,elapsed:f32)->SweptMotion{
  ...
  // Positional projection is not a velocity source. A deep correction could
  // otherwise inject far more speed than the rigid wheel actually has.
  let angular_velocity=delta/max(elapsed,1.0e-6);
  let arm=position-center;
  let surface=angular_velocity*vec2<f32>(-arm.y,arm.x);
