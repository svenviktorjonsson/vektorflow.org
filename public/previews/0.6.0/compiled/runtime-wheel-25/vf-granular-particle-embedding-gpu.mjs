...
fn grain_vertex(@builtin(vertex_index) vertex_index: u32,
  ...
  output.position = vec4<f32>(world_to_clip(world_position),
    0.0, 1.0);
  output.local = local;
  let displacement = length(grain.position - grain.previous_position);
  let airborne = !particle_mode && grain.contact_count == 0u
    && grain.velocity.y < -0.12;
  output.area_scale = select(1.0,
    1.0 - smoothstep(physical_radius * 2.0, physical_radius * 8.0,
      displacement) * 0.78, airborne);
  output.id = grain.id;
...
fn closed_density_at(uv: vec2<f32>) -> f32 {
  ...
  return (center + field_at(uv - vec2<f32>(offset.x, 0.0)).x
    + field_at(uv + vec2<f32>(offset.x, 0.0)).x
    + field_at(uv - vec2<f32>(0.0, offset.y)).x
    + field_at(uv + vec2<f32>(0.0, offset.y)).x) * 0.2;
}
...
fn material_composite_fragment(input: FullscreenOut) -> @location(0) vec4<f32> {
  ...
  let free_flow = smoothstep(0.12, 0.65, length(field_velocity))
    * (1.0 - smoothstep(0.8, 6.0, field.x));
  ...
  let flowing_seed = field.w / max(density, 1.0e-5);
...
fn advect_visual(@builtin(global_invocation_id) invocation: vec3<u32>) {
  ...
  let field_velocity = field.yz / max(field.x, 1.0e-5);
  // Visual grains may disperse only from an actually falling, contact-free
  // guide. A sideways velocity spread integrated along a gravity-driven fall
  // makes a cone of parabolic trajectories; an isotropic kick makes a sphere
  // and can lift sand that the physical Law has already settled.
  let falling = guide.contact_count == 0u && guide.velocity.y < -0.12
    && field.x < 2.4;
  let airborne = select(0.0, 1.0, falling);
  let signed_spread = visual_hash_unit(fine.id ^ 0x51a7d39bu) * 2.0 - 1.0;
  let spread = (0.09 + 0.13 * min(-guide.velocity.y, 2.0))
    * sqrt(visual_hash_unit(fine.id ^ 0x9e3779b9u));
  let horizontal = mix(guide.velocity.x, field_velocity.x,
    min(field.x * 0.1, 0.25)) + signed_spread * spread;
  var velocity = select(guide.velocity,
    vec2<f32>(horizontal, guide.velocity.y), falling);
  let settled = guide.contact_count > 0u && length(guide.velocity) < 0.10;
  let landed = guide.contact_count > 0u && fine.contact_count == 0u
    && fine.velocity.y < -0.12;
  let returned_to_rest = settled && length(fine.velocity) > 0.25;
  var next = followed + (velocity - guide.velocity) * params.visual.x;
  let displacement = next - guide.position;
  let limit = params.visual.y * mix(1.35, 8.0, airborne);
  if (dot(displacement, displacement) > limit * limit) {
    next = guide.position + normalize(displacement) * limit;
  }
  // Keep the optical sample fixed through sub-grain contact jitter. Retain
  // the anchor until real guide displacement crosses a fraction of a grain.
  let anchor_shift = length(guide.position - fine.previous_position);
  let locked = settled && anchor_shift < params.visual.y * 0.35;
  if (locked) { next = fine.position; velocity = vec2<f32>(0.0); }
  // Change mineral facets once when material returns after a real fall or
  // slide. No clock term and no per-frame re-roll in an unmoving pile.
  if (landed || returned_to_rest) {
    fine.id = visual_hash_u32(fine.id ^ (guide.id * 0x9e3779b9u)
      ^ 0x85ebca6bu);
  }
  fine.position = next;
  if (!locked) { fine.previous_position = guide.position; }
...
export async function createGranularParticleEmbeddingGpu(deviceArgument, canvasArgument,
  ...
  const updateParams = (mode, time, deltaTime, wheelAngle) => {
    ...
    values.set([wheelAngle, segments.length, 0, 0], 48);
