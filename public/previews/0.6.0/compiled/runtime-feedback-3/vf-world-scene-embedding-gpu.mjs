...
struct Vertex { @location(0) p:vec3<f32>, @location(1) n:vec3<f32>, @location(2) color:vec4<f32>, @location(3) tag:f32, @location(4) compliance:f32, @location(5) emission:vec3<f32> };
struct Out { @builtin(position) clip:vec4<f32>, @location(0) p:vec3<f32>, @location(1) n:vec3<f32>, @location(2) color:vec4<f32>, @location(3) emission:vec3<f32> };
...
fn posed(v:Vertex)->Out {
  ...
  return Out(scene.vp*vec4<f32>(p,1.0),p,n,v.color,v.emission);
}
...
fn grass_pose(vertex:u32,id:u32)->Out {
  ...
  return Out(scene.vp*vec4<f32>(p,1.0),p,n,vec4<f32>(color,1.0),vec3<f32>(0.0));
}
...
@fragment fn material_fragment(v:Out)->@location(0) vec4<f32>{
  ...
  let diffuse=max(0.0,dot(n,l));let view=normalize(scene.eye.xyz-v.p);let sheen=pow(max(0.0,dot(n,normalize(l+view))),24.0)*0.04;
  let irradiance=scene.sun.w/max(dot(scene.sun.xyz-v.p,scene.sun.xyz-v.p),0.01);
  return vec4<f32>(v.emission+v.color.rgb*(vec3<f32>(0.18)+scene.flags.yzw*irradiance*diffuse*shadow)+vec3<f32>(sheen*shadow),1.0);
}
...
export async function createWorldSceneEmbeddingGpu(device,canvas,world,physics,meshes,shaderSource){
  ...
  const attributes=[{shaderLocation:0,offset:0,format:'float32x3'},{shaderLocation:1,offset:12,format:'float32x3'},{shaderLocation:2,offset:24,format:'float32x4'},{shaderLocation:3,offset:40,format:'float32'},{shaderLocation:4,offset:44,format:'float32'},{shaderLocation:5,offset:48,format:'float32x3'}];
  const make=(entry,shadowPass=false,particles=false)=>device.createRenderPipelineAsync({layout:device.createPipelineLayout({bindGroupLayouts:[shadowPass?shadowLayout:layout]}),vertex:{module,entryPoint:entry,buffers:entry.startsWith('mesh')||entry==='shadow_vertex'?[{arrayStride:64,attributes}]:[]},...(shadowPass?{}:{fragment:{module,entryPoint:particles?'parcel_fragment':'material_fragment',targets:[{format,...(particles?{blend:{color:{srcFactor:'src-alpha',dstFactor:'one-minus-src-alpha'},alpha:{srcFactor:'one',dstFactor:'one-minus-src-alpha'}}}:{})}]}}),primitive:{topology:particles?'triangle-strip':'triangle-list',cullMode:'none'},depthStencil:{format:'depth32float',depthWriteEnabled:!particles,depthCompare:'less-equal'}});
  ...
  return {render(encoder,camera,{grass=true,particles=false}={}){
    ...
    f.set(multiply(perspective(camera.fov,w/h),view(camera.pos,camera.target)),0);f.set(multiply(perspective(105,1),view(sun,target)),16);
