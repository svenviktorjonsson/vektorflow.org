...
export function prepareMechanicalInitialState(world,arenas,asset){
  ...
  const convert=(vertices,tag,compliance,transform)=>{
    const out=new Float32Array(vertices.length/10*16);
    for(let i=0;i<vertices.length/10;i++){const src=i*10,d=i*16;out.set(vertices.subarray(src,src+10),d);if(transform)transform(out,d);out[d+10]=tag;out[d+11]=compliance?compliance(out,d):0;}
    return out;
  };
  if(kind==='rigid'){
    ...
    const nodeArena=arenas.find(a=>a.layer.id===world.nodes_layer_id);initial.nodeCount=nodeArena.layer.count;initial.nodes=new Float32Array(initial.nodeCount*8);initial.geometry=new Float32Array(initial.nodeCount*8);
    ...
    for(const item of asset){const vertices=convert(item.vertices,-1,(v,d)=>Math.pow(Math.max(0,v[d+2])/8,2)*(item.id?.includes('foliage')?1.2:0.65),(v,d)=>{
  ...
  meshes.push({shadow:false,vertices:new Float32Array([-radius,-radius,-.012,0,0,1,...color,-1,0,0,0,0,0,radius,-radius,-.012,0,0,1,...color,-1,0,0,0,0,0,radius,radius,-.012,0,0,1,...color,-1,0,0,0,0,0,-radius,radius,-.012,0,0,1,...color,-1,0,0,0,0,0]),indices:new Uint32Array([0,1,2,0,2,3])});
  p.lights=[];
