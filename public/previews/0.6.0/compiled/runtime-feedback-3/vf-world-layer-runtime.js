(function(root, factory) {
  ...
  if (script && script.dataset.vfWorldWasm) api.boot({
    ...
    root.__vfWorldLayerError = String(error.stack || error);
    console.error('VKF World layer application',error);
    var box = document.createElement('pre'); box.setAttribute('role','alert');
    box.textContent = root.__vfWorldLayerError; document.body.appendChild(box);
  });
  ...
  async function boot(options) {
    ...
    var runtime = globalThis.VfCompiledRuntimeBridge.instantiateWasmRuntime({bytes:bytes,manifest:manifest});
    runtime.init();
  ...
  return {createPresentation:createPresentation,boot:boot};
});
